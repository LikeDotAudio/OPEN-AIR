...
| 2026-07-18 | add | — | `README.md` (rewritten), `contracts/README.md`, `BackEnd/ComProtocols/README.md`, `ui/README.md`, `Documents/Strategies/README.md` | (this commit) | Documentation restructure: shipped work graduated from plans into per-section READMEs. `Documents/Strategies/*` bannered HISTORICAL (Ledger + Validations stay active); `Documents/Audits/*` bannered SNAPSHOT with a per-finding resolution table |
| 2026-07-18 | add | — | `openair-dnssd` real implementation (mdns-sd meta-browse), orchestrator spawn, builder dnssd branch, un-stubbed status | (this commit) | Stub → real agent: 42 services on first live sweep; retained topics self-clean on service removal. Also: VISA KB path fix (walk-up + embedded), per-scan retained cleanup, builder category pruning |
