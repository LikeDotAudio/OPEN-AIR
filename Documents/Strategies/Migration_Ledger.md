...
| 2026-07-17 | add | — | `contracts/validate.baseline.json` + ratchet mode in `cli/validate.ts` + CI validate step | (this commit) | Step 5: ratchet armed — CI fails only on debt not in the baseline; kill-tested (probe file → exit 1, removed → exit 0). Baseline may only shrink |
