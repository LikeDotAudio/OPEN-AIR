...
# 6_Media_Buttons/Media_Buttons.py
...
class GenericInstrumentGui(ttk.Frame):
    ...
    def _construct_dynamic_gui(self):
        try:
            ...
            # Create a container frame that we can GRID, because LoaderOrchestrator insists on PACKing itself
            self.builder_container = tk.Frame(self)
            self.builder_container.grid(row=0, column=0, sticky="nsew")

            self.dynamic_gui = LoaderOrchestrator(
                parent=self.builder_container,
