...
def main():
    ...
    nmos_cpp_dir = os.path.join(script_dir, "nmos-cpp-master")
    dev_dir = os.path.join(nmos_cpp_dir, "Development")
    ...
    print("\n🛠️  Configuring CMake for NMOS Registry...")
    cmake_args = [
    ...
    print("\n🔨 Building NMOS Registry (nmos-cpp-registry)...")
    run_cmd(["cmake", "--build", build_dir, "--target", "nmos-cpp-registry", "-j", str(os.cpu_count() or 4)])
    
    # Locate binary
    binary_path = os.path.join(build_dir, "nmos-cpp-registry")
    if not os.path.exists(binary_path):
        binary_path = os.path.join(build_dir, "Release", "nmos-cpp-registry")
        
    if not os.path.exists(binary_path):
        print(f"❌ Error: Compiled registry not found at {binary_path}")
        sys.exit(1)
