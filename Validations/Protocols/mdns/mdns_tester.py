...
if __name__ == "__main__":
    cfg, _ = config("mdns")
    ap = argparse.ArgumentParser(prog="mdns_tester")
    ...
    print(f"👂 [mdns] browsing '{stype}' for {a.timeout}s…")
    zc = Zeroconf(); ServiceBrowser(zc, stype, L())
    try: time.sleep(a.timeout)
    finally: zc.close()
    print(f"✅ [mdns] found {found[0]} service(s)."); sys.exit(0 if found[0] else 2)
