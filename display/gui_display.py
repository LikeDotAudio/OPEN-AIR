...
class Application(ttk.Frame):
    ...
    def __init__(
        ...
        try:
            ...
            self._build_from_directory(
                path=pathlib.Path(__file__).parent, parent_widget=self
            )
    ...
    def _apply_styles(self, theme_name: str):
        ...
        selected_orange = THEMES[theme_name if theme_name in THEMES else DEFAULT_THEME][
            "button_style_toggle"
        ]["Button_Selected_Bg"]
        selected_fg = THEMES[theme_name if theme_name in THEMES else DEFAULT_THEME][
            "button_style_toggle"
        ]["Button_Selected_Fg"]

        hover_bg = THEMES[theme_name if theme_name in THEMES else DEFAULT_THEME][
            "button_style_toggle"
        ]["Button_Hover_Bg"]
        hover_fg = THEMES[theme_name if theme_name in THEMES else DEFAULT_THEME][
            "button_style_toggle"
        ]["Button_Hover_Fg"]
        ...
        toggler_unselected_bg = THEMES[
            theme_name if theme_name in THEMES else DEFAULT_THEME
        ]["button_style_toggler_unselected"]["background"]
        toggler_unselected_fg = THEMES[
            theme_name if theme_name in THEMES else DEFAULT_THEME
        ]["button_style_toggler_unselected"]["foreground"]
        toggler_hover_bg = THEMES[
            theme_name if theme_name in THEMES else DEFAULT_THEME
        ]["button_style_toggler_unselected"]["Button_Hover_Bg"]
        toggler_hover_fg = THEMES[
            theme_name if theme_name in THEMES else DEFAULT_THEME
        ]["button_style_toggler_unselected"]["Button_Hover_Fg"]
        ...
        style.map(
            "Custom.TogglerUnselected.TButton",
            background=[
                ("active", toggler_hover_bg),
                ("!active", toggler_unselected_bg),
            ],
            foreground=[
                ("active", toggler_hover_fg),
                ("!active", toggler_unselected_fg),
            ],
        )
        ...
        style.configure("TNotebook", background=colors["primary"], borderwidth=0)
    ...
    def _build_from_directory(self, path: pathlib.Path, parent_widget):
        ...
        if app_constants.global_settings["debug_enabled"]:
            debug_logger(
                message=f"🏗️ _build_from_directory for path: '{path}'", **_get_log_args()
            )
        ...
        try:
            if layout_type == "horizontal_split" or layout_type == "vertical_split":
                ...
                for panel_info in layout_data["panels"]:
                    ...
                    self._build_from_directory(
                        path=sub_dir_path, parent_widget=new_frame
                    )
                ...
                def configure_sash(event):
                    ...
                    if len(percentages) > 1 and total_percentage > 0:
                        sash_pos = (
                            int(event.width * (percentages[0] / total_percentage))
                            if orientation == tk.HORIZONTAL
                            else int(event.height * (percentages[0] / total_percentage))
                        )
                ...
                notebook.bind(
                    "<<NotebookTabChanged>>", self._handle_tab_visibility, add="+"
                )
                ...
                for sub_dir in sub_dirs:
                    ...
                    if not (
                        dir_prefix in ["left", "right", "top", "bottom"]
                        or dir_prefix.isdigit()
                        or sub_dir.name.startswith("child_")
                    ):
                        self._build_from_directory(
                            path=sub_dir, parent_widget=parent_widget
                        )
                ...
                for py_file in py_files:
                    ...
                    instance = self.module_loader.load_and_instantiate_gui(
                        path=py_file, parent_widget=parent_widget
                    )
                    if instance:
                        instance.pack(fill=tk.BOTH, expand=True)
                        if app_constants.global_settings["debug_enabled"]:
        ...
        try:
            ...
            if not getattr(selected_tab_frame, "is_populated", False):
                ...
                if build_path:
                    ...
                    self._build_from_directory(
                        path=build_path, parent_widget=selected_tab_frame
                    )
            ...
            if selected_tab_frame.winfo_children():
                ...
                if hasattr(content_widget, "_on_tab_selected") and callable(
                    getattr(content_widget, "_on_tab_selected")
                ):
                    if app_constants.global_settings["debug_enabled"]:
                        debug_logger(
