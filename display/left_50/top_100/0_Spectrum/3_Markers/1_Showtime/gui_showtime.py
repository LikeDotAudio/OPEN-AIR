...
from workers.Showtime.worker_showtime_on_marker_button_click import (
    on_marker_button_click,
)
...
app_constants.debug_logger
    ...
    def _apply_styles(self, theme_name):
        ...
        style.configure(
            "Custom.TButton", background=dark_grey, foreground=colors["text"]
        )
        style.map(
            "Custom.TButton",
            background=[("active", hover_bg), ("!active", dark_grey)],
    ...
    def _create_widgets(self):
        ...
        tune_button = ttk.Button(
            tune_frame,
            text="Tune",
            command=lambda: on_tune_request_from_selection(self),
        )
    ...
    def _on_tab_selected(self, event):
        ...
        if (
            event is None
            or event.widget.tab(event.widget.select(), "text") == "Showtime"
        ):
            if app_constants.global_settings["debug_enabled"]:
                debug_logger(
    ...
    def _create_zone_buttons(self):
        current_function = inspect.currentframe().f_code.co_name
        if app_constants.global_settings["debug_enabled"]:
            debug_logger(message="🟢️️️🟢 Creating Zone buttons.", **_get_log_args())
    ...
    def _create_group_buttons(self):
        ...
        if app_constants.global_settings["debug_enabled"]:
            debug_logger(
                message="🟢️️️🟢 Creating Group filter buttons.", **_get_log_args()
            )
    ...
    def _create_device_buttons(self):
        current_function = inspect.currentframe().f_code.co_name
        if app_constants.global_settings["debug_enabled"]:
            debug_logger(message="🟢️️️🟢 Creating Device buttons.", **_get_log_args())
        ...
        if self.selected_zone and self.selected_group:
            filtered_devices = self.grouped_markers[self.selected_zone][
                self.selected_group
            ]
            if app_constants.global_settings["debug_enabled"]:
                debug_logger(
            ...
            for group_name in self.grouped_markers[self.selected_zone]:
                filtered_devices.extend(
                    self.grouped_markers[self.selected_zone][group_name]
                )
            if app_constants.global_settings["debug_enabled"]:
                debug_logger(
        ...
        for i, row_data in enumerate(filtered_devices):
            ...
            button = ttk.Button(
                self.device_frame, text=button_text, style="Custom.TButton"
            )
