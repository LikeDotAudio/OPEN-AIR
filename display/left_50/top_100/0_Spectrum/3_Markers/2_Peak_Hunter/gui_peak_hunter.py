...
class MarkerPeakHunterGUI(ttk.Frame):
    ...
    def _init_ui(self):
        ...
        if app_constants.global_settings['debug_enabled']:
            debug_logger(
                message=f"🖥️🟢 Initializing the MarkerPeakHunterGUI.",
                **_get_log_args()
            )
        ...
        try:
            ...
            self.refresh_btn = ttk.Button(self.control_frame, text="🔄 Refresh Peaks", command=self.refresh_data)
            self.refresh_btn.pack(side="left", padx=5)
            
            self.export_btn = ttk.Button(self.control_frame, text="💾 Export CSV", command=self.export_to_csv)
            ...
            for col in self.columns:
                self.commands_table.heading(col, text=col, command=lambda c=col: self.sort_column(c, False))
    ...
    def sort_column(self, col, reverse):
        ...
        l = [(self.commands_table.set(k, col), k) for k in self.commands_table.get_children('')]
        try:
        ...
        self.commands_table.heading(col, command=lambda: self.sort_column(col, not reverse))
    ...
    def on_item_double_click(self, event):
        ...
        if item_id:
            ...
            if app_constants.global_settings['debug_enabled']:
                debug_logger(
                    message=f"🎯 Peak Selected: {values}",
                    **_get_log_args()
                )
