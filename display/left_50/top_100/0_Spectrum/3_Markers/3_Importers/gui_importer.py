...
try:
    ...
    def dummy_action(*args): print("⚠️ Worker not connected!")
...
class MarkerImporterTab(ttk.Frame, GuiTableCreatorMixin):
    ...
    def __init__(self, master=None, app_instance=None, config_data=None, *args, **kwargs):
    ...
    def _apply_styles(self, theme_name: str):
        ...
        style.configure('Action.TButton', background=colors["secondary"], foreground=colors["text"])
        style.map('Action.TButton', background=[('active', colors["hover_blue"])])
        
        style.configure('Treeview', background=colors["table_bg"], foreground=colors["table_fg"], 
                        fieldbackground=colors["table_bg"], bordercolor=colors["table_border"], borderwidth=colors["border_width"])
        style.configure('Treeview.Heading', background=colors["table_heading_bg"], foreground=colors["fg"])
        style.configure('Orange.TButton', background=colors.get("button_style_toggle", {}).get("background", "orange"), foreground="black")
    ...
    def _create_widgets(self):
        ...
        # --- LOAD FRAME ---
        load_markers_frame = ttk.LabelFrame(self, text="Load Markers (Temporal Injection)", padding=(5,5,5,5))
        ...
        create_btn(load_markers_frame, "Load CSV", lambda: load_markers_file_action(self))
        create_btn(load_markers_frame, "Load IAS HTML", lambda: load_ias_html_action(self))
        create_btn(load_markers_frame, "Load WWB.shw", lambda: load_wwb_shw_action(self))
        create_btn(load_markers_frame, "Load WWB.zip", lambda: load_wwb_zip_action(self))
        create_btn(load_markers_frame, "Load SB PDF", lambda: load_sb_pdf_action(self))
        create_btn(load_markers_frame, "Load SB V2.pdf", lambda: load_sb_v2_pdf_action(self))
        ...
        self.marker_table_container.pack(fill=tk.BOTH, expand=True, padx=DEFAULT_PAD_X, pady=DEFAULT_PAD_Y)
        ...
        # --- APPEND FRAME ---
        append_markers_frame = ttk.LabelFrame(self, text="Append Markers (Merge Timelines)", padding=(5,5,5,5))
        append_markers_frame.pack(fill=tk.X, padx=DEFAULT_PAD_X, pady=DEFAULT_PAD_Y)
        
        create_btn(append_markers_frame, "Append CSV", lambda: append_markers_file_action(self, self.marker_tree.editor))
        create_btn(append_markers_frame, "Append IAS HTML", lambda: append_ias_html_action(self, self.marker_tree.editor))
        create_btn(append_markers_frame, "Append WWB.shw", lambda: append_wwb_shw_action(self, self.marker_tree.editor))
        create_btn(append_markers_frame, "Append WWB.zip", lambda: append_wwb_zip_action(self, self.marker_tree.editor))
        create_btn(append_markers_frame, "Append SB PDF", lambda: append_sb_pdf_action(self, self.marker_tree.editor))
        create_btn(append_markers_frame, "Append SB V2.pdf", lambda: append_sb_v2_pdf_action(self, self.marker_tree.editor))
