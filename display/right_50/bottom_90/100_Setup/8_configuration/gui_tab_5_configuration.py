...
class InitialConfigurationTab(ttk.Frame):
    def __init__(self, master=None, *args, **kwargs):
        ...
        debug_logger(
            message=f"📕🟢 Initializing InitialConfigurationTab...",
            file=current_file,
        ...
        debug_logger(
            message=f"📕✅ InitialConfigurationTab initialized.",
            file=current_file,
            version=current_version,
            function=current_function,
        )
    ...
    def _create_widgets(self):
        ...
        current_function = inspect.currentframe().f_code.co_name
        debug_logger(
            message=f"📕🟢 Creating widgets for InitialConfigurationTab.",
            file=current_file,
            version=current_version,
            function=current_function,
        )
        ...
        control_frame.grid_columnconfigure(
            0, weight=1
        )  # Only one column needs to expand

        save_button = ttk.Button(
            control_frame,
            text="Save Config",
            command=self._save_program_configure_action,
            style="Green.TButton",
        )
        save_button.grid(row=0, column=0, padx=5, pady=5, sticky="ew")

        self.config_text_widget = scrolledtext.ScrolledText(
            self,
            wrap="word",
            height=25,
            bg="#2b2b2b",
        ...
        reload_button = ttk.Button(
            ...
            text="Reload Config",
            command=self._reload_config_action,
            style="Orange.TButton",
        )
        reload_button.pack(
            side=tk.LEFT, anchor=tk.SW, padx=5, pady=5
        )  # Pack to left, anchor to South-West

        debug_logger(
            message=f"📕✅ Widgets created.",
            file=current_file,
            version=current_version,
            function=current_function,
        )
    ...
    def _populate_config_table(self):
        ...
        current_function = inspect.currentframe().f_code.co_name
        debug_logger(
            message=f"📕🟢 Populating config table from a mock file.",
            file=current_file,
            version=current_version,
            function=current_function,
        )
...
"""
            ...
            debug_logger(
                message=f"📕✅ Config table populated successfully.",
                file=current_file,
                version=current_version,
                function=current_function,
            )
            ...
            debug_logger(
                message=f"📕🔴 Failed to populate config table. Error: {e}",
                file=current_file,
                version=current_version,
                function=current_function,
            )
    ...
    def _save_program_configure_action(self):
        ...
        current_function = inspect.currentframe().f_code.co_name
        debug_logger(
            message=f"📕🟢 Attempting to save mock config file...",
            file=current_file,
            version=current_version,
            function=current_function,
        )
        ...
        try:
            ...
            debug_logger(
                message=f"📕✅ Mock configuration saved. Arrr, the treasure be safely stowed! ⚓️",
                file=current_file,
                version=current_version,
                function=current_function,
            )
            ...
            debug_logger(
                message=f"📕🔴 Failed to save configuration. Error: {e}",
                file=current_file,
                version=current_version,
                function=current_function,
            )
    ...
    def _reload_config_action(self):
        ...
        current_function = inspect.currentframe().f_code.co_name
        debug_logger(
            message=f"📕🟢 Reloading config from mock disk...",
            file=current_file,
            version=current_version,
            function=current_function,
        )
        ...
        debug_logger(
            message=f"📕✅ Reloaded mock config. The map be fresh! 🗺️",
            file=current_file,
            version=current_version,
            function=current_function,
        )
