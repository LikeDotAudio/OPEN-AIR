 ...
 *
 * Reference-only params (not in the saved JSON) render in red (notSaved).
 */
(function () {
  const isHex = (v) => typeof v === 'string' && /^#([0-9a-f]{3}|[0-9a-f]{4}|[0-9a-f]{6}|[0-9a-f]{8})$/i.test(v);

  const rowStyle = (depth, notSaved) => ({
    display: 'flex', alignItems: 'center', gap: 6,
    padding: '2px 4px', paddingLeft: 6 + depth * 10,
    borderLeft: notSaved ? '2px solid #e5484d' : '2px solid transparent',
  });
  const labelStyleFor = (notSaved) => ({
    flex: '0 0 42%', fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
    color: notSaved ? '#e5484d' : '#9aa',
    fontStyle: notSaved ? 'italic' : 'normal',
  });
  ...
  window.OaEdPropertyLeaf = ({ label, value, onChange, depth = 0, options, notSaved = false, dimension = false }) => {
    ...
    const labelStyle = labelStyleFor(notSaved);
    ...
    if (dimension) {
      ...
      return (
        <div style={rowStyle(depth, notSaved)}>
