# IncDecButtons — increment / decrement

Plus/minus stepper buttons, the web port of Python's
`BuilderInputIncDecButtonsCreator`.

- **Defines (global):** `IncDecButtons`
- **Props:** `value`, `onChange`, `config` (`value_default`, step), `topic`, `nodeJson`
...
```json
{
  "input_inc_dec_buttons_Example": {
    "type": "_SmartIncDec",
    "label": "Count:",
    "notes": "Simple increment/decrement counter.",
    "domain": {
      "primary": {
        "value_default": 0
      }
