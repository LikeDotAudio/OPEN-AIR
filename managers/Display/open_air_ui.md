...
OPEN-AIR utilizes a "Filesystem-as-Architecture" approach, where the structure of the `display/` directory directly dictates the layout and hierarchy of the user interface.
...
The `Application` class (found in `gui_display.py`) initiates a recursive build starting from the `display/` root. This process is managed by the `DirectoryBuilderMixin`.
