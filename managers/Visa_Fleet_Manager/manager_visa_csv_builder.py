...
class VisaCsvBuilder:
    def __init__(self, json_path=VISA_FLEET_JSON_PATH, csv_dir=CSV_OUTPUT_DIR):
        ...
        debug_logger(message=f"Initializing with JSON Path: {self.json_path}", **_get_log_args())
    ...
    def build_csvs_from_json(self):
        ...
        """
        debug_logger(message="Starting CSV build process (per table)...", **_get_log_args())
        if not os.path.exists(self.json_path):
            debug_logger(message=f"JSON file not found at {self.json_path}", level="ERROR", **_get_log_args())
            return
        ...
        with open(self.json_path, 'r') as f:
            try:
                ...
                debug_logger(message=f"Error decoding JSON from {self.json_path}: {e}", level="ERROR", **_get_log_args())
                return
    ...
    def _write_table_to_csv(self, table_node, table_path):
        ...
        if not headers or not isinstance(data_dict, dict):
            debug_logger(message=f"Warning: Skipping path {'/'.join(table_path)} because it's not a valid table structure.", **_get_log_args())
            return
        ...
        debug_logger(message=f"Writing table for topic '{topic_string}' to '{csv_filepath}'...", **_get_log_args())
        ...
        try:
            ...
            debug_logger(message=f"  Successfully wrote {len(data_list)} rows.", **_get_log_args())
        except Exception as e:
            debug_logger(message=f"  Error writing CSV file {csv_filepath}: {e}", level="ERROR", **_get_log_args())
