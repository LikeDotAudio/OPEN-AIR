...
try:
    ...
    print(
        "Warning: 'workers.logger' not found. Using dummy logger for VisaJsonBuilder."
    )
...
class VisaJsonBuilder:
    def __init__(self):
        ...
        debug_logger(
            message=f"🛠️ VisaJsonBuilder initialized with {len(KNOWN_DEVICES)} known devices.",
            **_get_log_args(),
        )
    ...
    def save_inventory_to_json(self, inventory_data):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                mode="w", delete=False, dir=os.path.dirname(filepath), suffix=".tmp"
            ) as tmp_f:
            ...
            debug_logger(
                f"✅ Atomically saved fleet inventory to {filepath}", **_get_log_args()
            )
    ...
    def load_inventory_from_json(self):
        ...
        if os.path.exists(filepath):
            if os.path.getsize(filepath) == 0:  # Check if file is empty
                debug_logger(
                    f"ℹ️ Inventory file {filepath} is empty. Initializing empty inventory.",
                    **_get_log_args(),
                )
            ...
            try:
                ...
                debug_logger(
                    f"✅ Loaded and flattened fleet inventory from {filepath} with {len(flat_inventory)} devices.",
                    **_get_log_args(),
                )
                return flat_inventory
            except json.JSONDecodeError as e:
                debug_logger(
                    f"❌ Error decoding inventory JSON from {filepath}: {e}. Initializing empty inventory.",
                    **_get_log_args(),
                    level="ERROR",
                )
                ...
                debug_logger(
                    f"❌ Error loading inventory from JSON: {e}. Initializing empty inventory.",
                    **_get_log_args(),
                    level="ERROR",
                )
            ...
            debug_logger(
                f"ℹ️ No existing inventory file found at {filepath}. Initializing empty inventory and creating file.",
                **_get_log_args(),
            )
    ...
    def load_grouped_inventory_from_json(self):
        ...
        if os.path.exists(filepath):
            if os.path.getsize(filepath) == 0:  # Check if file is empty
                debug_logger(
                    f"ℹ️ Grouped inventory file {filepath} is empty. Returning empty dictionary.",
                    **_get_log_args(),
                )
                return {}
            try:
                with open(filepath, "r") as f:
                    grouped_inventory = json.load(f)
                debug_logger(
                    f"✅ Loaded raw grouped fleet inventory from {filepath}.",
                    **_get_log_args(),
                )
                return grouped_inventory
            except json.JSONDecodeError as e:
                debug_logger(
                    f"❌ Error decoding grouped inventory JSON from {filepath}: {e}",
                    **_get_log_args(),
                    level="ERROR",
                )
                ...
                debug_logger(
                    f"❌ Error loading grouped inventory from JSON: {e}",
                    **_get_log_args(),
                    level="ERROR",
                )
            ...
            debug_logger(
                f"ℹ️ No existing grouped inventory file found at {filepath}. Initializing empty grouped inventory and creating file.",
                **_get_log_args(),
            )
            self.save_inventory_to_json([])  # Create the file with an empty inventory
            return {}  # Return empty dict
    ...
    def save_query_response_to_json(self, serial, response, command, corr_id):
        ...
        try:
            ...
            debug_logger(
                f"✅ Saved query response for {serial} to {filepath}", **_get_log_args()
            )
        except Exception as e:
            debug_logger(
