...
try:
    ...
    print(
        "Warning: 'workers.logger' not found. Using dummy logger for VisaFleetSupervisor."
    )
...
from managers.Visa_Fleet_Manager.manager_visa_parse_idn import (
    parse_idn_string,
)  # Moved to dedicated parsing module
...
class VisaFleetSupervisor:
    ...
    def scan_and_manage_fleet(self):
        ...
        if not self.scan_lock.acquire(blocking=False):
            debug_logger(
                "💳 ℹ️ FleetSupervisor: Scan already in progress. Skipping new scan.",
                **_get_log_args(),
            )
            return
        ...
        try:
            ...
            debug_logger(
                "💳 🔍 FleetSupervisor: Starting comprehensive fleet scan...",
                **_get_log_args(),
            )
            ...
            if app_constants.SCAN_USB:
                debug_logger(
                    "💳 🔍 Discovering USB/Local devices...", **_get_log_args()
                )
                usb_resources = manager_visa_USB.discover_usb_devices(
                    self.resource_manager
                )
                debug_logger(
                    f"💳 🔍 USB/Local resources found: {usb_resources}",
                    **_get_log_args(),
                )
                ...
                debug_logger(
                    "💳 ⏩ Skipping USB device scan as per config.", **_get_log_args()
                )
            ...
            if app_constants.SCAN_IP_DIRECT:
                debug_logger(
                    f"💳 🔍 Processing {len(dedicated_ips)} potential Dedicated IPs...",
                    **_get_log_args(),
                )
                for ip in dedicated_ips:
                    potential_targets.append(
                        {"Type": "DEDICATED", "Resource": f"TCPIP::{ip}::INSTR"}
                    )
            else:
                debug_logger(
                    "💳 ⏩ Skipping Direct IP device scan as per config.",
                    **_get_log_args(),
                )
            ...
            if app_constants.SCAN_GATEWAYS:
                debug_logger(
                    f"💳 🔍 Processing {len(gateway_ips)} potential Gateway IPs...",
                    **_get_log_args(),
                )
                gateway_resources = manager_visa_Gateway.discover_gateway_devices(
                    gateway_ips
                )
                debug_logger(
                    f"💳 🔍 Gateway resources found: {gateway_resources}",
                    **_get_log_args(),
                )
                ...
                debug_logger(
                    "💳 ⏩ Skipping Gateway device scan as per config.",
                    **_get_log_args(),
                )

            debug_logger(
                f"💳 🔍 Final list of potential targets ({len(potential_targets)}): {potential_targets}",
                **_get_log_args(),
            )
            debug_logger(
                f"   ⏳ Pausing 2s for settling after discovery phases...",
                **_get_log_args(),
            )
            time.sleep(2.0)
            ...
            probed_devices_collection = manager_visa_Search.probe_devices(
                self.resource_manager, potential_targets
            )
            debug_logger(
                f"💳 🔍 Probed devices collection: {probed_devices_collection}",
                **_get_log_args(),
            )

            current_scanned_serials = set(
                probed_devices_collection.keys()
            )  # All successfully probed devices
            ...
            for device_identifier, device_entry in probed_devices_collection.items():
                ...
                if device_entry.get("status") == "Active":
                    if device_identifier not in self.device_proxies:
                        debug_logger(
                            f"💳 ✨ FleetSupervisor: New active device detected: {device_identifier}",
                            **_get_log_args(),
                        )
                        ...
                        if existing_proxy.resource_name != device_entry.get(
                            "resource_string"
                        ):
                            debug_logger(
                                f"💳 🔄 FleetSupervisor: Resource string updated for {device_identifier}. Old: {existing_proxy.resource_name}, New: {device_entry.get('resource_string')}. Updating proxy.",
                                **_get_log_args(),
                            )
                            existing_proxy.resource_name = device_entry.get(
                                "resource_string"
                            )  # Update resource name in proxy
                else:  # Device is unresponsive or not active
                    if device_identifier in self.device_proxies:
                        debug_logger(
                            f"💳 ℹ️ FleetSupervisor: Active device {device_identifier} is now unresponsive. Shutting down its proxy.",
                            **_get_log_args(),
                        )
            ...
            for serial in removed_serials:
                debug_logger(
                    f"💳 ℹ️ FleetSupervisor: Device {serial} is no longer detected. Shutting down its proxy.",
                    **_get_log_args(),
                )
                ...
                if serial in self.instrument_inventory:
                    del self.instrument_inventory[
                        serial
                    ]  # Remove from inventory if completely gone

            debug_logger(
                f"💳 ✅ FleetSupervisor: Scan and management cycle complete. {len(self.device_proxies)} proxies active.",
                **_get_log_args(),
            )
    ...
    def _connect_and_setup_device(
        self,
        proxy_instance: VisaProxyFleet,
        resource_name: str,
        idn_details: dict,
        idn_string: str,
    ):
        ...
        debug_logger(
            f"💳 🔵 FleetSupervisor: Initiating connection for {device_serial} ({resource_name})",
            **_get_log_args(),
        )
        ...
        try:
            ...
            inst.timeout = (
                manager_visa_Search.VISA_TIMEOUT
            )  # Use consistent timeout from search module
            ...
            if device_serial in self.instrument_inventory:
                self.instrument_inventory[device_serial].update({"status": "CONNECTED"})
            ...
            error_msg = (
                f"VISA IO Error connecting to {device_serial} ({resource_name}): {vioe}"
            )
            debug_logger(
                f"💳 ❌ FleetSupervisor: {error_msg}", **_get_log_args(), level="ERROR"
            )
            import traceback

            debug_logger(
                f"Traceback: {traceback.format_exc()}", **_get_log_args(), level="ERROR"
            )
            self.manager._notify_error(
                serial=device_serial, message=error_msg, command="connect"
            )
            ...
            error_msg = (
                f"Unexpected error connecting to {device_serial} ({resource_name}): {e}"
            )
            debug_logger(
                f"💳 ❌ FleetSupervisor: {error_msg}", **_get_log_args(), level="ERROR"
            )
    ...
    def _emit_inventory_update(self):
        ...
        debug_logger(
            f"💳 📡⬆️ FleetSupervisor: Emitted fleet inventory ({len(inventory_list)} devices) to manager.",
            **_get_log_args(),
        )
    ...
    def shutdown(self):
        ...
        debug_logger(
            "💳 ℹ️ FleetSupervisor: Shutting down fleet supervisor.", **_get_log_args()
        )
        ...
        debug_logger(
            "💳 ℹ️ FleetSupervisor: All managed proxies shut down.", **_get_log_args()
        )
