...
try:
    ...
    print(
        "Warning: 'workers.logger' not found. Using dummy logger for VisaFleetManager."
    )
...
from managers.Visa_Fleet_Manager.manager_visa_json_builder import (
    VisaJsonBuilder,
)  # Import new builder
from managers.Visa_Fleet_Manager.manager_fleet_mqtt_bridge import (
    MqttFleetBridge,
)  # Import MQTT bridge
...
class VisaFleetManager:
    def __init__(self):
        ...
        self._current_inventory = (
            self.json_builder.load_inventory_from_json()
        ...
        debug_logger(
            message=f"💳 ✅ VisaFleetManager initialized. Supervisor ready.",
            **_get_log_args(),
        )

    def set_callbacks(
        self, on_inventory_update, on_device_response, on_device_error, on_proxy_status
    ):
    ...
    def start(self):
        self._running = True
        debug_logger(
            "💳 Core: VisaFleetManager Started (Standalone Mode).", **_get_log_args()
        )
    ...
    def stop(self):
        ...
        ):  # Ensure supervisor exists before trying to shut it down
    ...
    def trigger_scan(self):
        ...
        self._publish_scan_status(
            "Complete", {"status": "ready", "num_devices": num_devices_found}
        )
    ...
    def wait_for_initial_scan(self, timeout=None):
        ...
        """
        debug_logger(
            "⏳ Waiting for initial VISA fleet scan to complete...", **_get_log_args()
        )
        ...
        if completed:
            ...
            debug_logger(
                "⚠️ Timed out waiting for initial VISA fleet scan.", **_get_log_args()
            )
        return completed
    ...
    def _publish_scan_status(self, status, payload):
        ...
        if self.mqtt_bridge and self.mqtt_bridge.is_connected:
            ...
            debug_logger(
                message=f"Published scan status '{status}' to topic '{topic}'",
                **_get_log_args(),
            )
    ...
    def _notify_inventory(self, inventory_data):
        ...
        self.cb_inventory(
            augmented_inventory
        )  # Still emit the flat list for other listeners if needed
        self.mqtt_bridge.publish_inventory(
            grouped_inventory
