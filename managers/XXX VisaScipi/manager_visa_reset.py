...
class VisaResetManager:
    ...
    def _setup_mqtt_subscriptions(self):
        ...
        try:
            ...
            debug_logger(
                message="💳 ✅ The reset manager did subscribe to its topics.",
                **_get_log_args(),
            )
    ...
    def _on_reset_request(self, topic, payload):
        ...
        try:
            ...
            if str(data.get("value")).lower() == "true":
                debug_logger(
                    message=f"💳 🔵 Command received: Soft Reset. Dispatching '{self.CMD_RESET_DEVICE}'.",
                    **_get_log_args(),
                )
            ...
            debug_logger(
                message=f"💳 ❌ Error processing reset request payload: {payload}. Error: {e}"
            )
            debug_logger(
