...
def list_visa_resources():
    ...
    try:
        ...
        debug_logger(
            message="Scanning all available VISA resources (USB, TCPIP, GPIB, ASRL/Serial)...",
            file=current_file,
            version=current_version,
            function=current_function_name,
        )
        debug_logger(
            message=f"Using VISA Backend: {backend_info}",
            file=current_file,
            version=current_version,
            function=current_function_name,
        )
        ...
        if USB_SUPPORT:
            debug_logger(
                message="✅ USB Dependency (pyusb) is installed.",
                file=current_file,
                version=current_version,
                function=current_function_name,
            )
            ...
            debug_logger(
                message="❌ USB Dependency (pyusb) is MISSING. USB device discovery may fail.",
                file=current_file,
                version=current_version,
                function=current_function_name,
            )
            debug_logger(
                message="   Action: Run 'pip install pyusb' and ensure 'libusb' is installed on your OS.",
                file=current_file,
                version=current_version,
                function=current_function_name,
            )
        ...
        if NETWORK_ALL_INTERFACES_SUPPORT:
            debug_logger(
                message="✅ Network Dependency (psutil) is installed (enables all interface scanning).",
                file=current_file,
                version=current_version,
                function=current_function_name,
            )
            ...
            debug_logger(
                message="🟡 Network Dependency (psutil) is MISSING. Discovery limited to default interface.",
                file=current_file,
                version=current_version,
                function=current_function_name,
            )
            debug_logger(
                message="   Action: Run 'pip install psutil'.",
                file=current_file,
                version=current_version,
                function=current_function_name,
            )
        ...
        if NETWORK_HISLIP_SUPPORT:
            debug_logger(
                message="✅ HiSLIP Dependency (zeroconf) is installed.",
                file=current_file,
                version=current_version,
                function=current_function_name,
            )
            ...
            debug_logger(
                message="🟡 HiSLIP Dependency (zeroconf) is MISSING. HiSLIP resource discovery is disabled.",
                file=current_file,
                version=current_version,
                function=current_function_name,
            )
            debug_logger(
                message="   Action: Run 'pip install zeroconf'.",
                file=current_file,
                version=current_version,
                function=current_function_name,
            )
        ...
        if resources:
            debug_logger(
                message=f"✅ Found {len(resources)} VISA Resource(s):",
                file=current_file,
                version=current_version,
                function=current_function_name,
            )
            for i, resource in enumerate(resources, 1):
                ...
                debug_logger(
                    message=f"  {i}. {resource} ({resource_type})",
                    file=current_file,
                    version=current_version,
                    function=current_function_name,
                )
            ...
            debug_logger(
                message="🟡 No VISA resources found on the system.",
                file=current_file,
                version=current_version,
                function=current_function_name,
            )
            debug_logger(
                message="Note: If devices are connected, check device power and physical connection.",
                file=current_file,
                version=current_version,
                function=current_function_name,
            )
        ...
        debug_logger(
            message=f"❌ Error: PyVISA backend library failed to load with '{backend_to_use}'.",
            file=current_file,
            version=current_version,
            function=current_function_name,
        )
        debug_logger(
            message="  Ensure 'pyvisa-py' is installed and its dependencies (like 'pyusb') are met.",
            file=current_file,
            version=current_version,
            function=current_function_name,
        )
        ...
        debug_logger(
            message=f"❌ UNEXPECTED ERROR during VISA scan: {type(e).__name__}: {e}",
            file=current_file,
            version=current_version,
            function=current_function_name,
        )
        if app_constants.global_settings["debug_enabled"]:
            debug_logger(message=f"🖥️🔴 VISA scan failed: {e}", **_get_log_args())
    return []
