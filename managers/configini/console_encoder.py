...
def configure_console_encoding(): # Removed _func and debug_log_func arguments
    ...
    if os.name == 'nt':
        try:
            debug_logger(message="▶️ Entering configure_console_encoding.", **_get_log_args())
            ...
            debug_logger(message="✅ Successfully reconfigured stdout encoding.", **_get_log_args())
            ...
            debug_logger(message="✅ Successfully reconfigured stderr encoding.", **_get_log_args())
        except AttributeError:
            debug_logger(message="🟡 sys.stdout/stderr.reconfigure not available (likely older Python version). Skipping.", **_get_log_args())
            # Fallback for older Python versions that don't have reconfigure
            pass
        except Exception as e:
            debug_logger(message=f"⚠️ Exception during console encoding reconfiguration: {e}", **_get_log_args())
            # Log the exception but continue if possible
            
    else:
        debug_logger(message="⏩ Not on Windows ('nt'), skipping console encoding reconfiguration.", **_get_log_args())
