...
from managers.configini.config_builder import (
    create_default_config_ini,
)  # Import to rewrite config
import os
import sys
import inspect
import subprocess
...
def _execute_pip_command(
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if action == ACTION_UNINSTALL:
                ...
                if ERROR_NOT_INSTALLED in stderr_lower:
                    console_print_func(
                        f"🟡 {package_name} was not installed. Skipping uninstall."
                    )
                    return True
                elif "permission denied" in stderr_lower or (
                    "cannot uninstall" in stderr_lower
                    and (
                        "installed by debian" in stderr_lower
                        or "no record file" in stderr_lower
                    )
                ):
                    console_print_func(
                        f"⚠️ Pip uninstall for {package_name} failed due to permission/system package management. Proceeding with install. Error: {result.stderr.strip()}"
                    )
                    return True  # Treat as non-critical failure for uninstall, allow install to proceed
                else:
                    console_print_func(
                        f"❌ Pip {action} failed for {package_name}. Error: {result.stderr.strip()}"
                    )
                    return False
            # --- END MODIFICATION ---
        ...
        console_print_func(
            f"❌ An unexpected error occurred during pip operation for {package_name}: {e}"
        )
        return False


def action_check_dependancies(
    console_print_func, debug_log_func, should_clean_install=False
):  # Add argument with default
    # Checks for required external library dependencies and forces a reinstall if found.
    current_function_name = inspect.currentframe().f_code.co_name
    current_file = (
        f"{os.path.basename(__file__)}"  # Define current_file locally for this function
    )
    ...
    console_print_func(
        f"🔍 Starting dependency check ({len(EXTERNAL_PACKAGES) + len(BUILTIN_PACKAGES)} modules required). Clean Install Mode: {should_clean_install}"
    )
    ...
    try:
        ...
        for friendly_name, import_name in EXTERNAL_PACKAGES.items():
            ...
            if is_installed and should_clean_install:  # Use should_clean_install here
                ...
                _execute_pip_command(
                    ACTION_UNINSTALL,
                    package_name_for_pip,
                    console_print_func,
                    debug_log_func,
                    current_file,
                    current_version,
                ...
                console_print_func(
                    f"❌ '{friendly_name}' is missing. Attempting install..."
                )
                if not _execute_pip_command(
                    ACTION_INSTALL,
                    package_name_for_pip,
                ...
                is_installed and not should_clean_install
            ):  # Use should_clean_install here
                # Scenario C: Installed, not in fresh mode -> Skip, treat as success
                console_print_func(
                    f"✅ Found '{friendly_name}'. Skipping refresh (Non-fresh mode)."
                )
...
def run_interactive_pre_check(
    console_print_func, debug_log_func, app_constants_instance
):
    ...
    initial_clean_install_mode = (
        app_constants_instance.CLEAN_INSTALL_MODE
    )  # Store initial state

    if initial_clean_install_mode:
        console_print_func(
            "💡 Clean install mode enabled as per config.ini. All external libraries will be reinstalled."
        )
    else:
        console_print_func(
            "💡 Verify and install missing libraries mode enabled as per config.ini."
        )

    if not action_check_dependancies(
        console_print_func, debug_log_func, initial_clean_install_mode
    ):
        sys.exit(1)
