...
class YakRxManager:
    ...
    def _setup_mqtt_subscriptions(self):
        ...
        debug_logger(
            message=f"✅ YakRxManager subscribed to '{topic}' for proxy responses.",
            **_get_log_args(),
        )
    ...
    def _on_rx_outbox_message(self, topic, payload):
        ...
        debug_logger(
            message=f"📥 Rx_Outbox message received on Topic: '{topic}', Payload: '{payload}'",
            **_get_log_args(),
        )
        ...
        try:
            ...
            if correlation_id and response_value:
                ...
                if command_context:
                    ...
                    command_details = command_context.get(
                        "command_details"
                    )  # This would be the 'outputs' part
                    ...
                    if path_parts and command_details:
                        ...
                        debug_logger(
                            message=f"❌ Incomplete command context retrieved for correlation_id: {correlation_id}",
                            **_get_log_args(),
                            level="ERROR",
                        )
                else:
                    debug_logger(
                        message=f"❌ No command context found for correlation_id: {correlation_id}. Cannot process response.",
                        **_get_log_args(),
                        level="ERROR",
                    )
            else:
                debug_logger(
                    message=f"❌ Missing 'response' or 'correlation_id' in Rx_Outbox payload: {payload}",
                    **_get_log_args(),
                    level="ERROR",
                )
            ...
            debug_logger(
                message=f"❌ Error processing Rx_Outbox message from '{topic}': {e}",
                **_get_log_args(),
                level="CRITICAL",
            )
    ...
    def process_response(self, path_parts, command_details, response):
        ...
        if app_constants.global_settings["debug_enabled"]:
            ...
            debug_logger(message=f"ℹ️ Path Parts: {path_parts}", **_get_log_args())
            debug_logger(
                message=f"ℹ️ Command Details: {orjson.dumps(outputs, indent=2)}",
                **_get_log_args(),
            )
            debug_logger(message=f"ℹ️ Raw Response: {response}", **_get_log_args())
        ...
        try:
            ...
            if path_parts == self.NAB_BANDWIDTH_TRIGGER_PATH and len(output_keys) >= 5:
                ...
                if output_keys[3].endswith("Time_s") and output_keys[4].endswith("On"):
                    ...
                    if key_at_index_3.startswith(
                        "Sweep_Time_s"
                    ) and key_at_index_4.startswith("Continuous_Mode_On"):
            ...
            debug_logger(
                message="✅ Response processed and all output values published to MQTT.",
                **_get_log_args(),
            )
