...
def ensure_compiled():
    ...
    print("🦀 Compiling oacorerouter_rs...")
    ...
    if os.path.exists(venv_python):
        ...
        subprocess.run([venv_python, "-m", "pip", "uninstall", "-y", "oacorerouter_rs"], cwd=crate_dir, check=False, env=env)
    
    # Also attempt to uninstall from system/user python to prevent shadowing
    subprocess.run([sys.executable, "-m", "pip", "uninstall", "-y", "oacorerouter_rs", "--break-system-packages"], cwd=crate_dir, check=False, env=env)
    ...
    try:
        ...
        print(f"❌ Failed to compile or import oacorerouter_rs: {e}")
        raise e
