# oaComNmos Module: SAP to NMOS Bridge
...
The `oaComNmos` module has undergone a significant refactoring to adopt a modular, well-organized structure aligned with the project's **12-subfolder standard**. This approach enhances maintainability, testability, and adheres to principles of loose coupling and high cohesion.
...
-   **`oaComNmos/`**: Root directory of the module.
    ...
    -   **`__init__.py`**: Makes `oaComNmos` a Python package.
...
2.  **Command Line**: Run the bridge from the project root using:
    ```bash
    python3 oaComNmos/Entry.py --registrar <NMOS_REGISTRAR_URL>
    ```
...
This modular design facilitates easier development, testing, and future expansion of the `oaComNmos` module.
