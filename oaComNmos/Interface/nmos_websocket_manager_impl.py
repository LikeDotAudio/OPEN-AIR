# oaComNmos/Interface/nmos_websocket_manager_impl.py
...
class NmosWebsocketManagerImplementation(tk.Frame, TransparencyMixin):
    ...
    def _setup_ui(self):
        ...
        self.ws_status_var = tk.StringVar(value="Status: STOPPED")
        self.ws_status_lbl = tk.Label(status_frame, textvariable=self.ws_status_var, font=("Helvetica", 12, "bold"), fg="#ff0000", bg=bg_color)
        self.ws_status_lbl.pack(side=tk.LEFT, padx=10)

        self.ws_info_var = tk.StringVar(value="URL: - | Clients: 0")
        ...
        self.start_btn = ttk.Button(ctrl_frame, text="START WEBSOCKET", command=self._start_ws)
