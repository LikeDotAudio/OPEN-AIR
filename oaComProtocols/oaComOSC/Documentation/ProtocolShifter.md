# oaComOSC Protocol Shifter Documentation
...
*   **Example**:
    ```json
    {
      ...
      "source": "oaComOSC",
      "ts": 1775681977.47,
      "meta": { "msg_type": "SPLICE_ACTION", "origin_source": "oaComOSC" }
    }
    ```

## 3. MQTT Received Message (Hub Egress)
*   **Format**: Broadcasted JSON payload received from the Hub (MQTT).
*   **Action**: The Spoke Manager performs an `origin_source` check to prevent reflection loops (ignores if `origin_source == "oaComOSC"`).
