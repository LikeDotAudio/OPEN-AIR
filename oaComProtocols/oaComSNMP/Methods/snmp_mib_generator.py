# oaComProtocols.oaComSNMP/Methods/snmp_mib_generator.py
#
...
class MibGenerator:
    ...
    def generate(base_oid, oid_map, organization="OPEN-AIR Project"):
        ...
        lines = [
            "OPENAIR-MIB DEFINITIONS ::= BEGIN",
            "",
            "IMPORTS",
            "    MODULE-IDENTITY, OBJECT-TYPE, enterprises FROM SNMPv2-SMI",
            "    DisplayString FROM SNMPv2-TC;",
            "",
            "openAirMIB MODULE-IDENTITY",
            f"    LAST-UPDATED \"{timestamp}Z\"",
            f"    ORGANIZATION \"{organization}\"",
            "    CONTACT-INFO \"Anthony Peter Kuzub\"",
            "    DESCRIPTION \"MQTT-Aligned OID Tree for OPEN-AIR.\"",
            f"    REVISION \"{timestamp}Z\"",
            "    DESCRIPTION \"Strictly unique path-aligned generation.\"",
            f"    ::= {{ enterprises {pen} }}",
            "",
            "v1 OBJECT IDENTIFIER ::= { openAirMIB 1 }",
            ""
        ]
        ...
        def write_nodes(level_dict):
            ...
            for part, node_data in sorted_items:
                ...
                if node_data["children"]:
                    ...
                    lines.append("    SYNTAX      DisplayString")
                    lines.append("    MAX-ACCESS  read-write")
