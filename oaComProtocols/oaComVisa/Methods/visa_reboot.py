...
class VisaRebootManager:
    ...
    def __init__(self, mqtt_connection_manager, subscriber_router, visa_proxy):
        ...
        matrix_log("comms", "visa", inspect.currentframe().f_code.co_name if "inspect" in globals() else "unknown", f"💳 🟢️️️🟢 Initiating the {self.current_class_name}. The enforcer of reboots is online!", "DEBUG")
        try:
    ...
    def _setup_mqtt_subscriptions(self):
        ...
        current_function_name = inspect.currentframe().f_code.co_name
        matrix_log("comms", "visa", inspect.currentframe().f_code.co_name if "inspect" in globals() else "unknown", f"💳 ▶️ {current_function_name} to subscribe to reboot topics.", "DEBUG")
        try:
            self.subscriber_router.subscribe_to_topic(
                topic_filter=self.TOPIC_REBOOT, callback_func=self._on_reboot_request
            )
            matrix_log("comms", "visa", inspect.currentframe().f_code.co_name if "inspect" in globals() else "unknown", "💳 ✅ The reboot manager did subscribe to its topics.", "SUCCESS")

        except Exception as e:
            if LOCAL_DEBUG:
                logger.exception("💳 ❌ Error in {current_function_name}")
                matrix_log("comms", "visa", inspect.currentframe().f_code.co_name if "inspect" in globals() else "unknown", f"💳 🟢️️️🔴 The subscription circuits are fried! The error be: {e}", "DEBUG")
    ...
    def _on_reboot_request(self, topic, payload):
        ...
        try:
            ...
            if str(data.get("value")).lower() == "true":
                matrix_log("comms", "visa", inspect.currentframe().f_code.co_name if "inspect" in globals() else "unknown", f"💳 🔵 Command received: Power Cycle. Dispatching '{self.CMD_REBOOT_DEVICE}'.", "DEBUG")
            ...
            matrix_log("comms", "visa", inspect.currentframe().f_code.co_name if "inspect" in globals() else "unknown", f"💳 🟢️️️🔴 The reboot sequence has short-circuited! The error be: {e}", "DEBUG")
