## [V3.3.9] - 2026-05-05
### Fixed
- **Fader Refactor & MQTT Integration:** Modernized `Fader.jsx` to use ES modules and integrated `useMqttState` hook to restore UI functionality and real-time MQTT value updates.
...
- **Blueprint Validation Fix:** Demoted `FileNotFoundError` (missing blueprint) in `JsonGuiHost` from `ERROR` to `WARNING`. This prevents CI/CD log clutter for handled validation failures while maintaining visibility in the UI.
- **Logging Standardization:** Standardized all logs in `oaGui/Core/loader/json_gui_host.py` to use `matrix_log` with the "exactly three emojis" visual grepping rule and bracketed categories (e.g., `[VALIDATION]`, `[BUILDER]`, `[SUCCESS]`, `[CATASTROPHIC]`).
...
- **Layout Caching:** Integrated `cache_layout_store.json` for lightweight UI state management, separate from the functional device state cache.
...
- Added synchronous shutdown() method to LoaderShutdownService to handle non-GUI-event-driven termination.

## [20260404.2245.1] - 2026-04-04
- Fixed redundant traceback logging in Bootstrap sequence.
- Standardized shutdown calls in LoaderBootstrapEngine using root.after.
## [20260404.2300.1] - 2026-04-04
- Fixed X11 BadValue (0x0) crashes during UI build and background sync.
- Implemented robust dimension checks in LoaderOrchestrator, BuilderBackgroundManager, SyncBehavior, and OverlayManager.
...
- Hardened `JsonGuiHost` in `oaGui/Core/loader/json_gui_host.py` with 1x1 floor and `try...except` wrapper during builder instantiation.
...
### Fixed
- **Vertical Rendering:** Fixed a bug in LoaderOrchestrator where it only tracked width changes. It now correctly reacts to height changes, ensuring the preview fills the vertical space.
...
### Fixed
- **LoaderOrchestrator Vertical Height**: Fixed a critical bug in  where it was using stale  instead of the current event height. Also updated  to pass both dimensions to the resize handler. This ensures that GUI components fill the full vertical space in the main application.
...
## [20260429.0125.1] - 2026-04-29
### Fixed
- **LoaderOrchestrator Vertical Height**: Fixed a critical bug in `_perform_canvas_resize` where it was using stale `winfo_height()` instead of the current event height. Also updated `_on_canvas_configure` to pass both dimensions to the resize handler. This ensures that GUI components fill the full vertical space in the main application.
...
### Added
- **Global Telemetry Footer**: Implemented a real-time dimension footer for all `LoaderOrchestrator` windows. This footer displays the current `Viewport` (telemetry size) and `Content` (scrollable frame size) dimensions.
...
### Fixed
- **Footer Initialization**: Added an initial resize pass in the `_on_visibility` handler of `LoaderOrchestrator` to ensure the footer is populated with non-zero dimensions as soon as the UI is physical.
### Added
...
### Fixed
- **Footer Sync in Notebooks**: Renamed visibility handler to `_on_gui_visible` to ensure compatibility with `TabOrchestratorMixin`. This ensures that footer dimensions are correctly updated when switching tabs in a `Notebook` (e.g., the Zoo window).
...
### Fixed
- **Telemetry Discrepancy**: Updated `InteractionTelemetryService` to report actual widget (viewport) dimensions instead of the `toplevel` window size. This ensures MQTT telemetry aligns perfectly with the GUI footer display.
...
- **Bin-Level Footer Sync**: Updated `oaGui/Core/engine_structural_assembler.py` to trigger the builder's `_update_footer` during structural resizing. This ensures that nested containers (bins) report their viewport and content dimensions to the telemetry footer.
### Added
...
### Fixed
- **Grid Expansion Integrity**: Corrected multiple instances where `tk.Frame` containers were defaulting to restricted sizes. Forced `grid_rowconfigure(0, weight=1)` on all tab frames and universal loaders to ensure the `LoaderOrchestrator` can expand to fill the full vertical space.
...
### Fixed
- **Container Height Collapse**: Eliminated a hardcoded 200px height default in `engine_structural_assembler.py` for `OcaBin` objects. This allows containers to respect parent expansion weights and fill the full vertical height of the screen.
- **Grid Expansion Reliability**: Replaced the `ttk.Frame` main content container in `LoaderOrchestrator` with a standard `tk.Frame`. This bypasses style-based height clamping and ensures that `grid_rowconfigure(0, weight=1)` is respected by the OS rendering engine.

## [20260429.0185.1] - 2026-04-29
### Fixed
- **TclError (Unknown Option -bg)**: Resolved a crash in the `LoaderOrchestrator` by changing its inheritance from `ttk.Frame` to `tk.Frame`. This allows the builder to accept standard background configurations and ensures full compatibility with the industrial background engine.

## [20260429.0190.1] - 2026-04-29
### Fixed
- **Telemetry Event Gating**: Implemented strict event source validation across `LoaderOrchestrator`, `StructuralAssembler`, and `InteractionTelemetryService`. This prevents "Event Bubbling" where child widgets (like the footer or buttons) were accidentally triggering the parent's resize logic with their own small dimensions.
...
### [20260502.0835.1] - Stability Update
- RESOLVED systemic import failures across oaGui and oaGuiElements.
- FIXED SyntaxErrors in module entry points.
- RESTORED TUI responsiveness in oaTests.
- IMPLEMENTED legacy aliases for DeferredTaskHandler.
### [20260502.0840.1] - Stability Fix
- FIXED critical import failure for FolderRecursiveScannerMixin in EngineGuiDisplay orchestrator.
- RESTORED UI launch sequence.

## [2026-05-02 09:15] - Naming Convention Refactor
### Refactored
- **Systemic**: Renamed `self.config_data` to `self.configuration` across the entire `oaGui` module.
- **Constants**: Introduced `LayoutConstants` in `oaGui/Constants/builder_constants.py` to replace magic numbers.
- **Core Logic**: Renamed single-letter variables (`r`, `c`, `m`, etc.) in `engine_grid_layout_logic.py`, `command_router.py`, `yak_monitor.py`, and `coordinate_transformer.py`.
- **Methods**: Renamed generic functions (`defer`, `track`, `merge`) to more descriptive names while maintaining legacy aliases.
- **Consolidation**: Cleaned up duplication in `command_router.py` and `yak_monitor.py`.
