...
1.  **Rearrange Folders:** Move folders within the `oaGui/Assets/` directory. For example, moving a folder from `left_50/top_10` to `right_50/top_10` will move that entire UI block to the right column.
...
- **Default Panels:** If a JSON GUI doesn't specify a background, it defaults to the configuration in `oaGui/Assets/default_panel.json`.
