# Methods/utils_csv_writer.py
# Author: Anthony Peter Kuzub
# Version: 20250821.200641.1
#
# Description: This module provides utility functions for writing spectrum scan data to CSV files.
...
def write_scan_data_to_csv(
    ...
    if LOCAL_DEBUG:
        logger.debug(f"Attempting to write scan data to CSV: {file_path}, append_mode={append_mode}. Let's save this data!",
            file=__file__,
    ...
    if output_dir and not os.path.exists(output_dir):
        try:
            ...
            if LOCAL_DEBUG:
                logger.success(f"✅🔨 directory: {output_dir}. Path cleared!",
                    file=__file__,
                    version=current_version,
                    function=current_function,
                )
    ...
    try:
        ...
        with open(file_path, mode, newline="") as csv_file:
            ...
            if write_header:
                ...
                if LOCAL_DEBUG:
                    logger.debug(f"Wrote header to CSV file: {file_path}. Header added!",
                        file=__file__,
                        version=current_version,
                        function=current_function,
                    )
        ...
        # WRAPPED WITH after() to prevent cross-thread access
        app_instance_ref.after(
            0,
            lambda: console_print_func(
                f"✅ Scan data written to CSV: {file_path}. Data saved!"
            ),
        )
        if LOCAL_DEBUG:
            logger.debug(f"Scan data written to CSV: {file_path}. Mission accomplished!",
                file=__file__,
                version=current_version,
                function=current_function,
            )
