...
class HorizontalFaderRendererMixin:
    ...
    def render(self):
        ...
        cap_scale = float(self.config_data.get("fader_cap_scale", 1.0))
        DEFAULT_CAP_WIDTH = 50
        DEFAULT_CAP_HEIGHT = 55
        cap_width = int(float(self.config_data.get("cap_width", DEFAULT_CAP_WIDTH)) * cap_scale)
        cap_height = int(float(self.config_data.get("cap_height", DEFAULT_CAP_HEIGHT)) * cap_scale)
        ...
        self.canvas.create_line(padding_x, center_y, padding_x, center_y, fill=self.config_data.get("value_highlight_color", accent_color), width=TRACK_LINE_WIDTH, tags="fill_line")
        
        self._draw_ticks(canvas_width, canvas_height, center_y, padding_x)
        
        self.cap_img = HorizontalFaderAssetGenerator.get_3d_cap(cap_width, cap_height, self.config_data.get("cap_color", "#dcdcdc"), "#111", highlight_color=self.config_data.get("cap_highlight_color"))
    ...
    def _draw_ticks(self, canvas_width, canvas_height, center_y, padding_x):
        ...
        tick_interval = self.config_data.get("tick_interval")
        ...
        tick_color = self.config_data.get("tick_color", "light grey")
        sub_tick_color = self.config_data.get("sub_tick_color", "#555")
    ...
    def _update_positions(self, *args):
        ...
        DEFAULT_CAP_WIDTH = 50
        CAP_MARGIN_X = 10.0
        padding_x = (int(float(self.config_data.get("cap_width", DEFAULT_CAP_WIDTH)) * cap_scale)) / 2.0 + CAP_MARGIN_X
