# oaGui/FileReaders/standardizers/json_json_json_schema_harmonizer.py
...
class SchemaHarmonizer:
    """
    Normalizes "Universal Rhyme" schema into the flat schema expected by concrete widget creators.
    """

    def _harmonize_widget_config(self, config):
        if not isinstance(config, dict):
            return config

        # 0. Homogenized Schema & Shorthand Processing
        config = self._process_shorthand_lexicon(config)

        # 1. Style Inheritance
        styles_registry = getattr(self, "configuration", {}).get("styles", {})
    ...
    def _process_shorthand_lexicon(self, config):
        config = JsonShorthandResolver.resolve(config)
        pillars = ["identity", "geometry", "domain", "dynamics", "cosmetics"]
        if any(p in config for p in pillars):
            for p in pillars:
                if p in config:
                    block = JsonShorthandResolver.resolve(config.pop(p))
                    config.update(block)
        return config
