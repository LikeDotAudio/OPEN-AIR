...
class GuiWidgetFactoryMixin:
    ...
    Relies entirely on RegistryWidgetStore and dynamic discovery.
    """
    ...
    def _initialize_widget_factory(self):
        ...
        factory = RegistryWidgetStore.get_registry()
    ...
    def _lazy_wrap(self, module_path, class_name, method_name):
        ...
        def wrapper(parent_widget, configuration, context: WidgetContext = None, **kwargs):
            ...
            if hasattr(cls_ref, 'make'):
                return cls_ref.make(parent_widget, configuration, context=context, **kwargs)
            ...
            return method(builder_instance, parent_widget, configuration, context=context, **kwargs)
