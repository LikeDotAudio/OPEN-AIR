# Managers/interaction_navigation.py
...
class InteractionNavigationMixin:
    ...
    def show_splinker_tab(self, src_topic=None, dest_topic=None):
        ...
        """
        target_path = pathlib.Path(self.app_constants.GLOBAL_PROJECT_ROOT) / "oaGui/Assets/right_50/bottom_90/4_Splinker"
        target_frame = self._frames_by_path.get(target_path)

        if not target_frame:
            target_frame = self._frames_by_path.get(pathlib.Path("oaGui/Assets/right_50/bottom_90/4_Splinker"))
                ...
                if src_topic or dest_topic:
                    def _update_dashboard():
                        ...
                        for child in target_frame.winfo_children():
                            ...
                            while queue:
                                curr = queue.pop(0)
                                if hasattr(curr, 'set_pending_topics') and callable(curr.set_pending_topics):
