# oaGui/Methods/blueprint_normalizer.py
        ...
        if "fields" in config:
            ...
            if isinstance(fields, dict):
                for key, field in fields.items():
                    fields[key] = BlueprintNormalizer.normalize(field, root)
            elif isinstance(fields, list):
                for i, field in enumerate(fields):
                    fields[i] = BlueprintNormalizer.normalize(field, root)

        elif "blocks" in config:
            blocks = config["blocks"]
            if isinstance(blocks, dict):
                for key, block in blocks.items():
                    blocks[key] = BlueprintNormalizer.normalize(block, root)
            elif isinstance(blocks, list):
                for i, block in enumerate(blocks):
                    blocks[i] = BlueprintNormalizer.normalize(block, root)
            ...
            for key, value in config.items():
                if isinstance(value, dict):
                    ...
                    if key not in ["background", "styles", "style", "behavior",
                        ...
                        config[key] = (
                            BlueprintNormalizer.normalize(value, root)
                        )
                elif isinstance(value, list) and key in ["items", "blocks", "fields"]:
                     for i, item in enumerate(value):
                         value[i] = BlueprintNormalizer.normalize(item, root)
