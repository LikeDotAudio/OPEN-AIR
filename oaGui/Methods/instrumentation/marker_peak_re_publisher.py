...
TOPIC_MARKERS_ROOT = "OpenAir/measurements"
TOPIC_MARKER_NAB_OUTPUT_WILDCARD = (
    "OpenAir/yak/Markers/nab/NAB_all_marker_settings/Outputs/Marker_*/value"
)
NUMBER_OF_MARKERS = 6
...
class MarkerPeakPublisher:
    ...
    def __init__(self, mqtt_util: MqttControllerUtility, starting_device_id: str):
        ...
        logger.success(f"✅ Peak Publisher for {starting_device_id} is active and ready to catch peak values."
        )
    ...
    def _generate_device_map(self, start_id: str) -> dict:
        ...
        if not match:
            logger.error(f"❌ Error: Invalid starting Device ID format: {start_id}"
            )
            return {}
