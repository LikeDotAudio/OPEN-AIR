...
class GridRendererUtils:
    ...
    def resolve_fields(data):
        ...
        # ⚡ SINGLE WIDGET: If the data itself defines a widget type, return it as a list item
        if fields is None and data.get("type"):
            return [data]

        # ⚡ ANONYMOUS CONTAINER: If no container and no type, assume the dict is a collection of blocks
        if fields is None:
            fields = data
