...
#
# Description: oaGuiDefinitions/window_manager.py
...
class WindowManager:
    ...
    ):  # Removed current_version, debug_log_func
        self.application = (
    ...
    def tear_off_tab(self, event):
        ...
        try:
            ...
            # ⚡ LEAK PREVENTION: Destroy children of the original frame so hidden widgets
            # (like Splinker logs) stop processing in the background.
            # The frame itself remains so it can be re-attached and lazy-loaded later.
            for child in original_tab_frame.winfo_children():
                child.destroy()
            original_tab_frame.is_populated = False
...
# self.window_manager = WindowManager(self, app_constants.current_version, app_constants.LOCAL_DEBUG,  debug_logger)
#
