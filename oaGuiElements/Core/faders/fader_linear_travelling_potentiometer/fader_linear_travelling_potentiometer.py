...
class CustomLTPFrame(tk.Frame, LTPRendererMixin, LTPInteractionMixin):
    def __init__(self, master, config, path, state_mirror_engine, subscriber_router, base_mqtt_topic):
        ...
        # 3. State
        self.linear_var = kwargs.get("linear_variable") or tk.DoubleVar(master=parent_widget, value=float(f_cfg.get("value_default", 0.0)))
        self.rotation_var = kwargs.get("rotation_variable") or tk.DoubleVar(master=parent_widget, value=float(k_cfg.get("rotation_default", 0.0)))
