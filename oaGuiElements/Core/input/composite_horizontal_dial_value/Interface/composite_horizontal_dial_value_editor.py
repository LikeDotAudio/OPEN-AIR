...
class CompositeHorizontalDialValueEditor:
    """Standalone editor for composite_horizontal_dial_value configuration."""
    ...
    def __init__(self, parent, config_data, on_save_callback):
        ...
        self.window.title(f"Editor: {self.current_config.get('path', 'composite_horizontal_dial_value')}")
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="COMPOSITE HORIZONTAL DIAL VALUE EDITOR", bg="#333333", fg="white", 
                 font=("Arial", 10, "bold")).pack(side="left", padx=10, pady=10)

        container = tk.Frame(self.window, bg="#1e1e1e")
        container.pack(fill="both", expand=True, padx=10, pady=10)
        
        tk.Label(container, text="Bespoke properties for composite_horizontal_dial_value will appear here.", bg="#1e1e1e", fg="white").pack(pady=20)
    ...
    def launch(parent, config_data, on_save_callback):
        return CompositeHorizontalDialValueEditor(parent, config_data, on_save_callback)
