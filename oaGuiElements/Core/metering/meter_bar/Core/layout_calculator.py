...
class MeterLayoutCalculator:
    ...
    def calculate(self, w: int, h: int, configuration) -> LayoutResult:
        ...
        if not configuration.is_vertical:
            ...
            tick_start = bar_x + b_thick if configuration.scale_position in ["bottom", "right"] else bar_x
            tick_dir = 1 if configuration.scale_position in ["bottom", "right"] else -1
        ...
        # 4. Zones
        z1_norm = (configuration.middle_range - configuration.min_val) / (configuration.max_val - configuration.min_val)
