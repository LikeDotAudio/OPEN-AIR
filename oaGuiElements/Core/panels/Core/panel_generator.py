...
from .layer_dust import DustLayer
from .layer_metal_fold import MetalFoldLayer
from .layer_rust import RustLayer
from .layer_scratches import ScratchLayer
from .layer_screws import ScrewLayer
from .layer_stains import StainsLayer
from .layer_vignette import VignetteLayer
...
class PanelGenerator:
    ...
    @staticmethod
    def generate_procedural_panel(width, height, configuration_data=None):
        """
        Generates a PIL Image based on detailed physical parameters.
        Includes disk caching to prevent redundant generation.
        """
        # ⚡ ROBUSTNESS: Handle missing or invalid configuration_data
        if not configuration_data or not isinstance(configuration_data, dict):
            configuration_data = DEFAULT_PANEL_CONFIG
        ...
        # --- 0. Check Cache First ---
        matrix_log("gui", "gui_builder", "generate_procedural_panel", f"Checking for procedural panel in cache: {width}x{height} (Scale: {scale_factor})", "TRACE")
        cached_image = CacheImageStore.load_from_cache("panel", width, height, configuration_data)
        if cached_image:
            matrix_log("gui", "gui_builder", "generate_procedural_panel", "Retaining procedural panel from disk cache.", "DEBUG")
            return cached_image
        ...
        matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, f"Generating NEW Procedural Panel ({width}x{height}) Scale: {scale_factor} Seed: {random_seed}", "info".upper())
        ...
        matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, f"Layer 1. Substrate: {substrate_color_hex}", "trace".upper())
        ...
        if texture_type == "hammered":
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Applying hammered texture.", "trace".upper())
            ...
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Applying wrinkle noise texture.", "trace".upper())
            ...
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Weaving crosshatch streaks.", "trace".upper())
            ...
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Applying enamel peel texture.", "trace".upper())
            ...
            panel_image = ImageChops.soft_light(panel_image, peel_texture)
             ...
             matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Using flat substrate.", "trace".upper())
        else:
            streak_sigma = STREAK_SIGMA_BRUSHED if texture_type == "brushed" else STREAK_SIGMA_DEFAULT
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, f"Applying {texture_type} streak texture.", "trace".upper())
        ...
        matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, f"Layer 2. Paint: {paint_layer_settings.get('color')} (Opacity: {paint_opacity})", "trace".upper())
        paint_mask_image = Image.new('L', (width, height), MAX_OPACITY_VALUE)

        if edge_wear_settings.get("enabled", False):
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Etching edge wear scratches.", "trace".upper())
        ...
        if surface_scratch_count > 0 and panel_scratch_settings.get("reveals_substrate", False):
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Carving deep substrate-revealing scratches.", "trace".upper())
        ...
        if gradient_intensity > 0:
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Applying linear lighting gradient.", "trace".upper())
        ...
        if haze_settings.get("enabled", False):
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Layer 3. Infusing warm studio haze.", "trace".upper())
        ...
        if rust_settings.get("enabled", False):
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Layer 4. Spawning rust oxidation spots.", "trace".upper())
        ...
        if edge_wear_settings.get("enabled", False) and vignette_intensity_value > 0:
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Layer 5. Applying vignette edge fade.", "trace".upper())
        ...
        if int(panel_scratch_settings.get("count", 0)) > 0:
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Layer 6. Adding surface micro-scratches.", "trace".upper())
        ...
        if int(grime_settings.get("stain_count", 0)) > 0:
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Layer 7. Adding grease and coffee stains.", "trace".upper())
        ...
        if screws_settings.get("enabled", False):
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Layer 8. Drilling screws into the panel.", "trace".upper())
        ...
        if fold_settings.get("enabled", False):
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Layer 8. Folding metal creases.", "trace".upper())
        ...
        if dust_settings.get("enabled", False):
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Layer 8. Settling fine dust particles.", "trace".upper())
        ...
        if blur_radius_value > 0:
            matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, f"Applying global Gaussian blur: {blur_radius_value}", "trace".upper())
            panel_image = panel_image.filter(ImageFilter.GaussianBlur(radius=blur_radius_value))

        matrix_log("gui", "gui_builder", inspect.currentframe().f_code.co_name, "Procedural panel generation complete. Saving to disk cache.", "success".upper())
        CacheImageStore.save_to_cache("panel", width, height, configuration_data, panel_image)
        return panel_image

    @staticmethod
    def generate_tkinter_panel_image(width, height, configuration_data=None):
        ...
        try:
            ...
            # ⚡ FALLBACK: Return a solid dark grey panel if generation fails completely
            fallback_img = Image.new('RGBA', (width, height), (40, 40, 40, 255))
