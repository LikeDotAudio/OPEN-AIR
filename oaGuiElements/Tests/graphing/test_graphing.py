...
class TestGraphPlotter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            self.patchers.append(patch('oaGuiElements.Core.graphing.Methods.graph.FigureCanvasTkAgg', return_value=mock_canvas_agg))
            for p in self.patchers:
                p.start()
        # Path to the component directory
        component_dir = os.path.join(os.path.dirname(__file__), '..', '..', 'Core', 'graphing')
