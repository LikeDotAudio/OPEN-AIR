...
#
# Description: Compiler hook for oalogginggate_rs. 
...
def ensure_compiled():
    ...
    try:
        ...
        wheels = glob.glob(os.path.join(module_dir, "target", "wheels", "oalogginggate_rs-*.whl"))
        if not wheels:
            # Try with hyphens replaced by underscores or vice versa
            normalized = "oalogginggate_rs".replace("_", "-")
