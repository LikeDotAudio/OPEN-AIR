...
class TestHardwareBoundary(unittest.TestCase):
    def test_visa_timeout_handling(self):
        ...
        with patch("oaComVisa.Visa_Scipi_dialog.visa_proxy.query_safe", side_effect=Exception("Timeout")):
...
class TestFileAndEnvironment(unittest.TestCase):
    @patch("oaOchestration.path_initializer.pathlib.Path.mkdir")
...
class TestIntegrationRoundTrip(unittest.TestCase):
    def test_end_to_end_knob_to_visa(self):
        ...
        with patch("oaTranslator.core.value_processor.ValueProcessor.extract_value", return_value=75.0), \
             patch("oaTranslator.core.value_processor.ValueProcessor.normalize", return_value=75.0):
