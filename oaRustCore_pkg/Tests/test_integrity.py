            ...
            for f in files:
                if f.endswith(".so") or (f.endswith(".pyd") and os.name == 'nt'):
                    if f.endswith(".so"):
                        ...
                        # Skip the build package and root-level orchestrator link
                        if str_rel_path.startswith("oaRustCore_pkg") or (str_rel_path == "." and f == "oaRustCore.so"):
                            continue
                        # Skip .venv
                        if str_rel_path.startswith(".venv"):
                            continue
