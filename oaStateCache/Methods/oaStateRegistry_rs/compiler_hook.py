...
def ensure_compiled():
    ...
    if os.path.exists(venv_site):
        for item in os.listdir(venv_site):
            if item.startswith("~") and "oastateregistry_rs" in item:
    ...
    if os.path.exists(venv_python):
        subprocess.run([venv_python, "-m", "pip", "uninstall", "-y", "oastateregistry_rs"], cwd=module_dir, check=False, env=env)
    else:
        subprocess.run([sys.executable, "-m", "pip", "uninstall", "-y", "oastateregistry_rs"], cwd=module_dir, check=False, env=env)
