# CleanupUtilities/cleaupflamegraph.py
...
def cleanup_flamegraph():
    ...
    logger.info(f"🧹 Starting Flame Graph cleanup...")
    ...
    if os.path.exists(target):
        ...
        if files_purged > 0:
            logger.info(f"   ✅ Purged {files_purged} items from: {os.path.relpath(target, project_root)}")
        else:
