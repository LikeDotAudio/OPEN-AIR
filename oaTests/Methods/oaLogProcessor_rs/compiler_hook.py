...
#
# Description: Compiler hook for oalogprocessor_rs. 
...
def ensure_compiled():
    ...
    try:
        ...
        wheels = glob.glob(os.path.join(module_dir, "target", "wheels", "oalogprocessor_rs-*.whl"))
        if not wheels:
            # Try with hyphens replaced by underscores or vice versa
            normalized = "oalogprocessor_rs".replace("_", "-")
