...
from oaLogging.log_filter_engine import initialize_filter_engine
...
def launch_core_managers(state_cache_manager, mqtt_connection_manager):
    ...
    # Core Infrastructure
    splinker_module_path = "oaSplinker.splinker"
    if importlib.util.find_spec(splinker_module_path):
        ...
        logger.critical("❌ Critical module missing: oaSplinker.splinker")
        return None
    ...
    if importlib.util.find_spec(visa_fleet_module_path):
        ...
        logger.critical("❌ Critical module missing: oaComVisa.Visa_Fleet.visa_fleet")
        return None
    
    yak_translator_module_path = "oaTranslator.yak_translator"
    if importlib.util.find_spec(yak_translator_module_path):
        ...
        logger.critical("❌ Critical module missing: oaTranslator.yak_translator")
        return None
    
    yak_rx_module_path = "oaTranslator.yak_rx"
    if importlib.util.find_spec(yak_rx_module_path):
        ...
        logger.critical("❌ Critical module missing: oaTranslator.yak_rx")
        return None
    
    fleet_status_module_path = "oaWatchdog.fleet_status_monitor"
    if importlib.util.find_spec(fleet_status_module_path):
        ...
        logger.critical("❌ Critical module missing: oaWatchdog.fleet_status_monitor")
        return None
    
    ptp_module_path = "oaPTP.ptp"
    if importlib.util.find_spec(ptp_module_path):
        ...
        logger.critical("❌ Critical module missing: oaPTP.ptp")
        return None
