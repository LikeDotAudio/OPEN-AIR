class TopicCalculator:
    ...
    def calculate(self, widget_id, tab_name):
        ...
        filtered_parts = [p for p in parts if p not in ["display", "gui", "oaGuiDefinitions", "oaGuiElements"]]
        ...
        if (widget_id_str.startswith(self.base_topic + "/") or 
            ...
            clean_tab_parts = [p for p in tab_parts if p not in ["display", "gui", "oaGuiDefinitions", "oaGuiElements"]]
