...
from managers.Visa_Fleet.visa_fleet import FleetOrchestrator

class TestVisaFleet(unittest.TestCase):
    def setUp(self):
        self.mqtt = MagicMock()
        self.router = MagicMock()
        with patch("managers.Visa_Fleet.visa_fleet.DiscoveryOrchestrator"), \
             patch("managers.Visa_Fleet.visa_fleet.VisaJsonBuilder.load_inventory_from_json", return_value=[]):
