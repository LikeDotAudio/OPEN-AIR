 ...
 *
 * Props: { value, onPaste }
 *   value   — the layout object to copy (the saved instance value)
 *   onPaste — (layoutObj) => void, applied when pasting (e.g. store.setProp(...,'layout',v))
 *
 * Uses the system clipboard (pretty JSON) with an in-memory fallback
 * (`window.OaEdLayoutClip`). Accepts a copied WHOLE element too (plucks its `.layout`).
 */
(function () {
  window.OaEdCopyLayout = ({ value, onPaste }) => {
    ...
    const copy = async (e) => {
      ...
      window.OaEdLayoutClip = JSON.parse(JSON.stringify(value));
      // Wrap under a `layout` parent so paste can tell a layout from a style.
      try { await navigator.clipboard.writeText(JSON.stringify({ layout: value }, null, 2)); } catch (_) { /* in-memory fallback */ }
      flash('copied');
    };
    const paste = async (e) => {
      stop(e);
      ...
      if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
        if (parsed.layout && typeof parsed.layout === 'object') v = parsed.layout;        // wrapped layout / whole element
        else if (parsed.style && typeof parsed.style === 'object') { flash("that's a style"); return; } // wrong kind
        else v = parsed;                                                                  // bare object (legacy/manual)
      }
      if (!v && window.OaEdLayoutClip) v = window.OaEdLayoutClip;                          // clipboard-blocked fallback
    ...
    return (
      <span onClick={stop} style={{ display: 'inline-flex', gap: 4, alignItems: 'center', marginLeft: 'auto' }}>
        <button style={b} onClick={copy} title="Copy layout to clipboard">⧉ copy</button>
        <button style={b} onClick={paste} title="Paste layout from clipboard">⇩ paste</button>
