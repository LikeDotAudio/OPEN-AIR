    ...
    React.useEffect(() => {
        ...
        const processQueue = (deadline) => {
            while (currentIndex < queue.length && (deadline.timeRemaining() > 2 || deadline.didTimeout)) {
                ...
                // Handler only — see oaConfigForBus in MqttProvider.jsx. This
                // idle crawl walks the whole tree, so it is what creates most of
                // the retained config topics; most widgets it visits have no
                // yak_handler and now publish no config at all.
                const busConfig = window.oaConfigForBus(item.config);
                if (busConfig) publish(`${item.topic}/config`, JSON.stringify(busConfig));
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', width: '100vw', height: '100vh', backgroundColor: '#000', color: '#fff' }}>
            ...
            <div style={{ display: 'flex', backgroundColor: '#111', borderBottom: '1px solid #333', alignItems: 'center', padding: '0 10px', height: '35px', flexShrink: 0 }}>
                ...
                <a
                    href="../Documentation/5_Manual/index.html"
                    target="_blank"
                    rel="noopener noreferrer"
