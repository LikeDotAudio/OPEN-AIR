...
class MarkerGoGetterWorker:
    ...
    def __init__(self, mqtt_util: MqttControllerUtility):
        ...
        """
        if LOCAL_DEBUG: logger.debug(f"🟢️️️🟢 Initializing the tireless Marker Go-Getter!")
    ...
    def _on_marker_data_update(self, topic, payload):
        ...
        try:
            ...
            logger.debug(f"🟡 Warning: Could not process marker data update from topic '{topic}': {e}"
            )
    ...
    def _handle_start_stop(self, topic, payload):
        ...
        try:
            ...
            if is_start_command and (
                ...
                logger.debug("🟢 START command received. Beginning marker peak acquisition loop."
                )
                ...
                logger.debug("🔴 STOP command received. Halting marker peak acquisition loop."
                )
                self.stop_event.set()
            ...
            logger.error(f"❌ Error processing start/stop command: {e}")
    ...
    def _place_markers_for_batch(self, batch_ids):
        ...
        for j, device_id in enumerate(batch_ids, 1):
            ...
            if LOCAL_DEBUG: logger.debug(f"🐐🔵 Place Marker {j}: {device_id} sent {freq_mhz} MHz ({freq_hz} Hz).",
                file=current_file,
        ...
        logger.debug("🟠 Recovering after Marker Placement to clear potential downstream crash..."
        )
    ...
    def _query_markers_for_batch(self, batch_ids):
        ...
        if LOCAL_DEBUG: logger.debug("🔵 Sending NAB query to retrieve current peak markers...")
        ...
        if LOCAL_DEBUG: logger.debug("🟠 Waiting for NAB query and publishing to complete...")
        #time.sleep(0.2)  # A minimal, safe wait to ensure messages hit the system.

        if LOCAL_DEBUG: logger.success(f"✅ Peak retrieval process initiated for batch: {', '.join(batch_ids)}."
        )
    ...
    def _set_instrument_frequency_span(self):
        ...
        if (
            ...
            logger.debug("🟢️️️🟡 Min/Max frequencies unchanged. Skipping span update.", file=current_file,
                version=current_version,
                function=current_function_name,
            )
            return
        ...
        if LOCAL_DEBUG: logger.debug(f"🔵 Setting instrument span from {new_min_freq} MHz to {new_max_freq} MHz (with {BUFFER_START_STOP_MHZ} MHz buffer)."
        )
        self.mqtt_util.publish_message(
        ...
        while not self.stop_event.is_set():
            ...
            for i in range(0, len(device_ids), 6):
                if self.stop_event.is_set():
                    logger.debug("Loop terminated by STOP command during batch processing."
                    )
                    break
                ...
                logger.success(f"✅ Batch {i//6 + 1} processed. Continuing to next batch."
                )

            if LOCAL_DEBUG: logger.success("✅ Peak Hunter loop finished a full pass.")
...
def Push_Marker_to_Center_Freq(mqtt_controller, marker_data):
    ...
    logger.debug("🟢️️️🟢 Received request to tune to marker. Processing data...", file=current_file,
        version=current_version,
    ...
    try:
        ...
        if freq_mhz is None:
            logger.error("❌🔴 Error: Marker data is missing the 'FREQ_MHZ' key.", file=current_file,
                version=current_version,
                function=f"{current_function}",
            )
            logger.error("❌ Failed to tune: Marker data is incomplete.")
            return
        ...
        try:
            ...
            logger.error(f"❌🔴 Error converting frequency to float: {e}",
                file=current_file,
                version=current_version,
                function=f"{current_function}",
            )
            logger.error("❌ Failed to tune: Invalid frequency value.")
            return

        logger.debug(f"🔍 Freq from marker: {freq_mhz} MHz -> {center_freq_hz} Hz. Setting Span to {DEFAULT_SPAN_HZ} Hz.",
            file=current_file,
            version=current_version,
            function=f"{current_function}",
        )
        ...
        if LOCAL_DEBUG: logger.success(f"✅ Set CENTER_FREQ to {center_freq_hz} Hz.")

        mqtt_controller.publish_message(
            topic=SPAN_FREQ_TOPIC, subtopic="", value=int(DEFAULT_SPAN_HZ)
        )
        if LOCAL_DEBUG: logger.success(f"✅ Set SPAN_FREQ to {int(DEFAULT_SPAN_HZ)} Hz.")

        mqtt_controller.publish_message(topic=TRIGGER_TOPIC, subtopic="", value=True)
        logger.debug("🟢️️️🔵 Trigger set to True. Awaiting instrument response.", file=current_file,
            version=current_version,
            function=f"{current_function}",
        )

        mqtt_controller.publish_message(topic=TRIGGER_TOPIC, subtopic="", value=False)
        logger.debug("🟢️️️🔵 Trigger reset to False. Command sequence complete.", file=current_file,
            version=current_version,
            function=f"{current_function}",
        )

        if LOCAL_DEBUG: logger.success("✅ Tuning command sequence complete.")

    except Exception as e:
        if LOCAL_DEBUG:
            logger.exception("❌🔴 Critical error during marker tuning",
                file=current_file,
                version=current_version,
                function=f"{current_function}",
            )
            logger.exception("❌ An error occurred while tuning to the marker")
...
def Push_Marker_to_Start_Stop_Freq(mqtt_controller, marker_data, buffer=1e6):
    ...
    """
    current_function = inspect.currentframe().f_code.co_name
    logger.debug(f"🟢️️️🟢 Received request to tune with a buffer. Buffer is {buffer} Hz. Processing data...",
        file=current_file,
        version=current_version,
        function=f"{current_function}",
    )
    ...
    try:
        ...
        logger.debug(f"🔍 Calculated range: Start={start_freq_hz} Hz, Stop={stop_freq_hz} Hz.",
            file=current_file,
            version=current_version,
            function=f"{current_function}",
        )
        ...
        if LOCAL_DEBUG: logger.success(f"✅ Set START_FREQ to {start_freq_hz} Hz.")

        # Publish stop frequency
        mqtt_controller.publish_message(
            topic=STOP_FREQ_TOPIC, subtopic="", value=stop_freq_hz
        )
        if LOCAL_DEBUG: logger.success(f"✅ Set STOP_FREQ to {stop_freq_hz} Hz.")
        ...
        if LOCAL_DEBUG:
            logger.exception("❌🔴 Critical error during marker tuning with buffer",
                file=current_file,
                version=current_version,
                function=f"{current_function}",
            )
            logger.exception("❌ An error occurred while tuning to the marker with a buffer"
            )
