# workers/builder/audio/dynamic_gui_create_custom_horizontal_fader.py
#
...
class CustomHorizontalFaderCreatorMixin:
    ...
    def _draw_horizontal_fader(self, frame_instance, canvas, width, height, value):
        canvas.delete("all")
        cy = height / 2
        canvas.create_line(10, cy, width - 10, cy, fill=frame_instance.track_col, width=4, capstyle=tk.ROUND)
        ...
        tick_values = frame_instance.ticks or range(int(frame_instance.min_val), int(frame_instance.max_val) + 1, 10)
        ...
        for i, tick_value in enumerate(tick_values):
            ...
            if 0.0 <= linear_tick_norm <= 1.0:
                display_tick_norm = linear_tick_norm ** (1.0 / frame_instance.log_exponent)
                tick_x_pos = (width - 20) * display_tick_norm + 10
                canvas.create_line(tick_x_pos, cy - tick_length_half, tick_x_pos, cy + tick_length_half, fill=tick_color, width=1)
                if i % 2 == 0:
                    canvas.create_text(tick_x_pos, cy + 15, text=str(int(tick_value)), fill=tick_color, anchor="n")
        ...
        self._draw_rounded_rectangle(
            canvas,
            handle_x - cap_width / 2, cy - cap_height / 2,
            handle_x + cap_width / 2, cy + cap_height / 2,
            radius=10,
        ...
        canvas.create_line(handle_x, cy - center_line_length/2, handle_x, cy + center_line_length/2, fill=frame_instance.track_col, width=2)

        # 60% lines at 25% and 75% of width
        side_line_length = cap_height * 0.6
        x_offset = cap_width * 0.25
        canvas.create_line(handle_x - x_offset, cy - side_line_length/2, handle_x - x_offset, cy + side_line_length/2, fill=frame_instance.track_col, width=1)
        canvas.create_line(handle_x + x_offset, cy - side_line_length/2, handle_x + x_offset, cy + side_line_length/2, fill=frame_instance.track_col, width=1)