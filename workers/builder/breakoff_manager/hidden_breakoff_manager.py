# hidden/hidden_breakoff_manager.py
#
...
from workers.Command_Router.mqtt.mqtt_publisher_service import is_connected
...
class HiddenBreakoffManagerMixin:
    ...
    def _setup_breakoff_snitch(self):
        ...
        if not self.state_mirror_engine or not self.state_mirror_engine.root:
            return

        # ⚡ OPTIMIZATION: Disable snitches in WYSIWYG/Editor mode
    ...
    def _check_breakoff_state(self, event=None):
        ...
        # If we didn't setup correctly, abort
    ...
    def _on_broken_off_configure(self, event):
        ...
        # ⚡ OPTIMIZATION: Only publish if the event is for the toplevel itself
    ...
    def _publish_breakoff_state(self):
        ...
        if self.is_broken_off and self.toplevel_window:
            try:
                # ⚡ OPTIMIZATION: Don't call update_idletasks() inside an event handler
                # as it can trigger re-entrancy issues and Tcl '??' errors.
