# workers/builder/dynamic_gui_create_bar_graph.py
...
class BarGraphCreatorMixin:
    def _create_bar_graph(
        self, parent_widget, config_data, **kwargs
    ):  # Updated signature
        """Creates a Bar graph widget that is state-aware."""
        current_function_name = "_create_bar_graph"
        ...
        if app_constants.global_settings["debug_enabled"]:
            debug_logger(
                message=f"🔬⚡️ Entering '{current_function_name}' to calibrate a Bar graph for '{label}'.",
                **_get_log_args(),
            )
        ...
        try:
            ...
            # Peak Display (Secondary pointer)
            peak_display = config.get("Peak_display", True)
            peak_display_fall_time = float(config.get("Peak_display_Fall_time", 1000))
            peak_display_colour = config.get("Peak_display_colour", "red")
            peak_display_style = config.get("Peak_display_line_style", "line") # line or rect
            ...
            canvas_bg = config.get("bg", bg_color) # Default to theme bg_color if not specified
            
            canvas = tk.Canvas(
                frame, width=total_width, height=total_height, bg=canvas_bg, highlightthickness=0
            )
            canvas.pack()
            ...
            # --- Draw Peak Display Indicator ---
            peak_indicator = None
            if peak_display:
                if peak_display_style == "line":
                    peak_indicator = canvas.create_line(
                        pad_x, bar_y, pad_x, bar_y + bar_height, fill=peak_display_colour, width=2
                    )
                else:
                    peak_indicator = canvas.create_rectangle(
                        pad_x, bar_y, pad_x + 5, bar_y + bar_height, fill=peak_display_colour, outline=""
                    )
            ...
            frame.anim_peak_expiry = 0
            frame.anim_peak_value = value_default
            ...
            def draw_indicator():
                ...
                # Draw Peak Indicator
                if peak_display and peak_indicator:
                    pv = frame.anim_peak_value
                    if pv < min_val: pv = min_val
                    if pv > max_val: pv = max_val
                    px_pos = (
                        (pv - min_val) / (max_val - min_val) * base_width
                        if max_val > min_val
                        else 0
                    )
                    if peak_display_style == "line":
                        canvas.coords(peak_indicator, pad_x + px_pos, y1, pad_x + px_pos, y2)
                    else:
                        canvas.coords(peak_indicator, pad_x + px_pos - 2.5, y1, pad_x + px_pos + 2.5, y2)
            ...
            def animate():
                ...
                # Handle Peak value ballistics
                if peak_display:
                    # If current value is higher than peak, peak follows it exactly
                    if current > frame.anim_peak_value:
                        frame.anim_peak_value = current
                    else:
                        # Otherwise, peak falls at its own rate
                        if peak_display_fall_time > 0:
                            p_step = (full_range / peak_display_fall_time) * dt
                            frame.anim_peak_value = max(min_val, frame.anim_peak_value - p_step)

                if frame.anim_mode == "holding":
                    if (time.time() * 1000) - frame.anim_hold_start >= hold_time:
                        frame.anim_mode = "decaying"
                    else:
                        draw_indicator() # Still update peak if it's falling
                ...
                # We should keep running as long as current != target OR peak is falling
                is_peak_falling = peak_display and frame.anim_peak_value > current and frame.anim_peak_value > min_val
                
                if abs(diff) < 0.05 and not is_peak_falling:
            ...
            if app_constants.global_settings["debug_enabled"]:
                debug_logger(
                    message=f"✅ SUCCESS! The Bar graph '{label}' is now registering on the Richter scale!",
                    **_get_log_args(),
                )
            ...
            if app_constants.global_settings["debug_enabled"]:
                debug_logger(
                    message=f"❌ The Bar graph for '{label}' has overloaded! Error: {e}",
                    **_get_log_args(),
                )
            return None