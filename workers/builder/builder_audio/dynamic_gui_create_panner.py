...
class PannerCreatorMixin:
    def _create_panner(self, parent_widget, config_data, **kwargs):
        ...
        try:
            ...
            def update_panner_visuals(*args):
                self._draw_panner_visuals(canvas, width, height, panner_value_var.get(), frame.min_val, frame.max_val, value_label, fg_color, accent_color, visual_props["secondary"])

            panner_value_var.trace_add("write", update_panner_visuals)
            update_panner_visuals()

            def on_mousewheel(event):
                current_val = panner_value_var.get()
    ...
    def _draw_panner_visuals(self, canvas, width, height, value, min_val, max_val, value_label, neutral_color, accent, secondary):
        canvas.delete("all")
        active_color, cx, cy = neutral_color, width / 2, height / 2
        radius, center_angle_tk, max_extent = min(width, height) / 2 - 5, 90, 135
        canvas.create_arc(cx - radius, cy - radius, cx + radius, cy + radius, start=225, extent=-270, style=tk.ARC, outline=secondary, width=4)
        norm_val = ((value - min_val) / (max_val - min_val) * 2) - 1 if max_val != min_val else 0
        if abs(norm_val) < 0.01:
            active_color = neutral_color
            canvas.create_line(cx, cy - radius + 2, cx, cy - radius + 12, fill=active_color, width=3, capstyle=tk.ROUND)
        else:
            active_color = "red" if norm_val > 0 else "white"
            extent = -1 * (norm_val * max_extent) if norm_val > 0 else abs(norm_val) * max_extent
            canvas.create_arc(cx - radius, cy - radius, cx + radius, cy + radius, start=center_angle_tk, extent=extent, style=tk.ARC, outline=active_color, width=4)
