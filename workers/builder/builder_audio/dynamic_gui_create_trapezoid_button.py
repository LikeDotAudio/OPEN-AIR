...
class TrapezoidButtonCreatorMixin:
    ...
    def _create_trapezoid_button(self, parent_widget, config_data, **kwargs): # Updated signature
        ...
        led_color = config.get("led_color", indicator_color) # Use specific led_color, fallback to general indicator_color
        ...
        if path and self.state_mirror_engine:
            widget_id = path
            self.state_mirror_engine.register_widget(widget_id, state_var, base_mqtt_topic_from_path, config)
    ...
    def _draw_trapezoid_button(self, canvas, config, state):
        ...
        if lit:
            ...
            # Glow effect
            canvas.create_oval(
                light_x - light_radius * 1.5, light_y - light_radius * 1.5,
                light_x + light_radius * 1.5, light_y + light_radius * 1.5,
            ...
            canvas.create_oval(
                ...
                light_x + light_radius, light_y + light_radius,
                fill=self._adjust_color(led_color, 1.5), outline=self._adjust_color(led_color, 2.0)
            )
