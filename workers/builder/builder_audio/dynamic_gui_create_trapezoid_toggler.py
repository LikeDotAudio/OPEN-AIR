...
class TrapezoidButtonTogglerCreatorMixin(TrapezoidButtonCreatorMixin):
    ...
    def _create_trapezoid_button_toggler(
        ...
        if app_constants.global_settings["debug_enabled"]:
            debug_logger(
                message=f"Creating trapezoid button toggler group: {label}",
                **_get_log_args(),
            )
        ...
        def on_state_change(*args):
            ...
            for key, button_info in buttons.items():
                ...
                self._draw_trapezoid_button(
                    button_info["canvas"], button_info["config"], button_info["state"]
                )
        ...
        for key, button_config in options.items():
            ...
            def on_press(event, k=key):
                ...
                self._draw_trapezoid_button(
                    buttons[k]["canvas"], buttons[k]["config"], buttons[k]["state"]
                )
