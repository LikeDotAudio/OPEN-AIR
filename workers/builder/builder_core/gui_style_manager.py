# builder_core/gui_style_manager.py
#
...
class GuiStyleMixin:
    ...
    # Applies a specified visual theme to the application's GUI.
    # This method configures the styles for various ttk widgets, such as frames, labels,
    # entries, and buttons, using a color palette and style properties defined in the THEMES dictionary.
    # It sets colors, fonts, and other visual attributes to ensure a consistent look and feel.
    # Inputs:
    ...
    def _apply_styles(self, theme_name):
        ...
        style.configure(
            ...
            background=bg,
            foreground=entry_fg,
            bordercolor=colors.get("border", "#555555"),
        )
        style.map(
            "TEntry",
            fieldbackground=[
                ("readonly", entry_bg),
                ("disabled", colors.get("fg_alt", "#888888")),
            ],
            background=[
                ("readonly", entry_bg),
                ("disabled", colors.get("fg_alt", "#888888")),
        ...
        style.map(
            ...
            background=[
                ("selected", accent),
                ("!selected", colors.get("secondary", bg)),
            ],
            foreground=[
            ...
            font=[
                ("selected", ("Helvetica", 14, "bold")),
                ("!selected", ("Helvetica", 12)),
            ],
        )  # +2 points
        ...
        style.configure(
            "Custom.Selected.TButton", background=accent, foreground=colors.get("fg")
        )
        style.map(
        ...
        style.map(
            "Custom.Treeview",
            background=[
                ("selected", colors.get("treeview_selected_bg", colors["accent"]))
            ],
        ...
        style.map(
            ...
            background=[
                (
                    "active",
                    colors.get("treeview_heading_active_bg", colors["secondary"]),
                )
            ],
        )
