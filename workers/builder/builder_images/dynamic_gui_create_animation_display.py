...
import os
from workers.setup.path_initializer import (
    GLOBAL_PROJECT_ROOT,
...
class AnimationDisplayCreatorMixin:
    ...
    def _create_animation_display(
        ...
        try:
            ...
            debug_logger(
                message=f"🔴 GIF not found at {gif_path_absolute}. Creating placeholder.",
                **_get_log_args(),
            )
            try:
                ...
                debug_logger(
                    message=f"☑️ INFO: Created placeholder image at {placeholder_filename}"
                )
            except Exception as e_placeholder:
                debug_logger(
                    message=f"🔴 ERROR creating placeholder image: {e_placeholder}",
                    **_get_log_args(),
                )
        ...
        def _update_frame(*args):  # Add *args to accept trace arguments
            try:
                ...
                if app_constants.global_settings["debug_enabled"]:
                    debug_logger(
                        message=f"🔴 ERROR updating animation frame: {e}",
    ...
    def _on_animation_frame_update_mqtt(self, topic, payload):
        ...
        try:
            ...
            expected_prefix = get_topic(
                self.state_mirror_engine.base_topic, base_mqtt_topic_from_path, ""
            )  # Construct expected prefix with new base topic
            if topic.startswith(expected_prefix):
                widget_path = topic[len(expected_prefix) :].strip(TOPIC_DELIMITER)
            else:
                if app_constants.global_settings["debug_enabled"]:
                    debug_logger(
                        message=f"⚠️ Unexpected topic format for animation frame update: {topic}",
                        **_get_log_args(),
                    )
                return
            ...
            full_topic = get_topic(
                self.state_mirror_engine.base_topic, base_mqtt_topic_from_path, widget_path
            )
            if full_topic in self.state_mirror_engine.registered_widgets:
                ...
                if app_constants.global_settings["debug_enabled"]:
                    debug_logger(
                        message=f"⚠️ Animation widget not found in registered_widgets for path: {widget_path}",
                        **_get_log_args(),
                    )
            ...
            if app_constants.global_settings["debug_enabled"]:
                debug_logger(
                    message=f"❌ Error processing animation MQTT update for {topic}: {e}. Payload: {payload}",
                    **_get_log_args(),
                )