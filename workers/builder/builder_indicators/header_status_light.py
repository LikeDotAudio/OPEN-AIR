...
class HeaderStatusLightMixin:
    ...
    def _build_header_status_light(
        ...
        self.status_canvas = tk.Canvas(
            self.header_frame, width=20, height=20, bg="#F0F0F0", highlightthickness=0
        )
        ...
        self.status_light_id = self.status_canvas.create_oval(
            2, 2, 18, 18, fill="red", outline="white"
        )

        # 3. Label (Optional) - extract from config
        label_text = config.get(
            "label_active", "Fleet Status:"
        ...
        if self.state_mirror_engine:
            self.state_mirror_engine.subscriber_router.subscribe_to_topic(
                "OPEN-AIR/GUI/Global/Header/StatusLight", self._update_status_light
            )
            self.state_mirror_engine.subscriber_router.subscribe_to_topic(
