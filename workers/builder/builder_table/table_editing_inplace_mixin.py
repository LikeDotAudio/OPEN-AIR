...
class TableEditingInplaceMixin:
    ...
    def commit_edit(self, new_value):
        ...
        if self.data_topic and device_key:
            ...
            self.state_mirror_engine.publish_payload(field_topic, orjson.dumps(row_data)) # publish_payload from state_mirror_engine
            debug_logger(
    ...
    def _on_entry_commit(self, event=None):
        ...
        if event and event.keysym == "Return" and (event.state & 0x0001):
            ...
            debug_logger(message=f"Auto-incremented value to: {new_value}", **_get_log_args())
