...
class TableEditingRowOpsMixin:
    ...
    def add_row(self):
        ...
        if self.data_topic and device_key:
            ...
            mqtt_publisher_service.publish_payload(
                field_topic, orjson.dumps(new_row_data)
            )
            debug_logger(
        ...
        debug_logger(
            message=f"➕ Added new row with item_id: {new_item_id}, device_key: {device_key}",
            **_get_log_args(),
        )
    ...
    def delete_selection(self, event=None):
        ...
        for item_id in selected_items:
            ...
            debug_logger(
                message=f"🗑️ Deleted row {item_id} (Device Key: {device_key}).",
                **_get_log_args(),
            )
    ...
    def import_data(self, data_list):
        current_function = inspect.currentframe().f_code.co_name
        debug_logger(
            message=f"➕ Importing {len(data_list)} new rows.", **_get_log_args()
        )
        ...
        for row_data_dict in data_list:
            ...
            if self.data_topic and device_key:
                field_topic = get_topic(self.data_topic, "data", device_key)
                mqtt_publisher_service.publish_payload(
                    field_topic, orjson.dumps(row_data_dict)
                )
                debug_logger(
        ...
        debug_logger(
            message=f"➕ Finished importing {len(data_list)} rows.", **_get_log_args()
        )