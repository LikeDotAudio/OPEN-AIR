        ...
        if last_action["action"] == "edit":
            ...
            if self.data_topic and device_key:
                ...
                mqtt_publisher_service.publish_payload(
                    field_topic, orjson.dumps(row_data_after_undo)
                )
                debug_logger(
            ...
            if self.data_topic and device_key:
                field_topic = get_topic(self.data_topic, "data", device_key)
                mqtt_publisher_service.publish_payload(
                    field_topic, orjson.dumps(old_row_data)
                )
                debug_logger(
            ...
            debug_logger(
                message=f"⏪ Undo: Re-inserted row for device {device_key}.",
                **_get_log_args(),
            )
            ...
            if self.tree.exists(item_id):
                ...
                debug_logger(
                    message=f"⏪ Undo: Removed added row with item_id: {item_id}, device_key: {device_key}.",
                    **_get_log_args(),
                )
