...
class HorizontalMeterWithText(ttk.Frame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if LOCAL_DEBUG: logger.debug(f"📊📈 HorizontalMeter '{self.widget_id}' initialized. GUID: {self.GUID}")
    ...
    def _on_meter_value_var_change(self, *args):
        ...
        if LOCAL_DEBUG: logger.trace(f"📉💹 HorizontalMeter '{self.widget_id}' updated to {new_value:.3f}.")
        # Update UI
...
class VerticalMeter(ttk.Frame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if LOCAL_DEBUG: logger.debug(f"📈📉 VerticalMeter '{self.widget_id}' initialized. GUID: {self.GUID}")
    ...
    def _on_meter_values_var_change(self, *args):
        ...
        if LOCAL_DEBUG: logger.trace(f"💹📊 VerticalMeter '{self.widget_id}' received fresh data: {new_values_str}")
        try:
            ...
            logger.debug(
                f"❌ Error processing meter_values_var: {e}")
