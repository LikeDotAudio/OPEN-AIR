...
class BezelGeometry:
    ...
    def get_bezel_points(cx, cy, w, h, shape, line_width, shrink_px=0):
        ...
        if shape_key == "gem": 
            ...
            y_base_shift = global_y_shift
            pts_user = [
                (0, 0 + y_base_shift),                                   
                (TRIANGLE_BASE_WIDTH * tri_rad, TRIANGLE_PEAK_HEIGHT * tri_rad + y_base_shift),  
                (-TRIANGLE_BASE_WIDTH * tri_rad, TRIANGLE_PEAK_HEIGHT * tri_rad + y_base_shift)  
            ]

        elif shape_key == "pyramid":
            py_rad = radius * PYRAMID_BEZEL_EXPANSION
            y_base_shift = global_y_shift
            pts_user = [
                (0, PYRAMID_PEAK_HEIGHT * py_rad + y_base_shift),      
                (PYRAMID_BASE_WIDTH * py_rad, 0 + y_base_shift),       
                (-PYRAMID_BASE_WIDTH * py_rad, 0 + y_base_shift)       
            ]
            
        elif shape_key in ["cylinder", "hotdog"]:
            if shape_key == "hotdog":
                w_straight = HOTDOG_WIDTH_STRAIGHT * radius
                h_total = HOTDOG_HEIGHT_TOTAL * radius
                r_cap = HOTDOG_CAP_RADIUS * radius 
                cap_center_y = HOTDOG_CAP_CENTER_Y * radius
            else:
                w_straight = CYLINDER_WIDTH_STRAIGHT * radius
                h_total = CYLINDER_HEIGHT_TOTAL * radius
                r_cap = CYLINDER_CAP_RADIUS * radius 
                cap_center_y = CYLINDER_CAP_CENTER_Y * radius
            
            pts_user.append((0, 0 + global_y_shift)) 
            pts_user.append((w_straight, 0 + global_y_shift))
            steps = CYLINDER_STEPS
            for i in range(steps+1):
                ang = math.radians(-90 + (180 * i/steps))
                px = (w_straight) + (r_cap * math.cos(ang))
                py = (cap_center_y) + (r_cap * math.sin(ang))
                pts_user.append((px, py + global_y_shift))
            for i in range(steps+1):
                ang = math.radians(90 + (180 * i/steps))
                px = (-w_straight) + (r_cap * math.cos(ang))
                py = (cap_center_y) + (r_cap * math.sin(ang))
            ...
            for i in range(steps + 1):
                ...
                pts_user.append((x, y_raw + h_sq + global_y_shift))

        elif shape_key == "squimonde":
            is_smooth = True
            n = SQUIRCLE_N
            ...
            for i in range(steps + 1):
                ...
                pts_user.append((x_rot, y_rot + h_sq + global_y_shift))

        elif shape_key == "squircle":
            is_smooth = True
            n = SQUIRCLE_N
            w_sq = SQUIRCLE_WIDTH_FACTOR * radius
            h_sq = SQUIRCLE_HEIGHT_FACTOR * radius
        ...
        elif shape_key in ["trapezoid", "badge"]:
            pts_user = [
                (0, 0 + global_y_shift),
            ...
            for i in range(1, curve_steps + 1):
                ...
                pts_user.append((x_u, y_u + global_y_shift))
            pts_user.append((CREST_TOP_WIDTH_FACTOR * radius, CREST_TOP_HEIGHT_FACTOR * radius + global_y_shift))
            pts_user.append((-CREST_TOP_WIDTH_FACTOR * radius, CREST_TOP_HEIGHT_FACTOR * radius + global_y_shift))
            pts_user.append((-CREST_TOP_WIDTH_FACTOR * radius, CREST_BOTTOM_HEIGHT_FACTOR * radius + global_y_shift))
        ...
        elif shape_key == "intersecting_overlay":
