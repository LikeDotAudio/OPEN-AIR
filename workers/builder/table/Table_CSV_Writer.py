...
class TableCsvWriter:
    def write_to_csv(self, file_path, headers, data):
        ...
        try:
            ...
            debug_logger(message=f"✅ Successfully wrote table data to {file_path}", **_get_log_args())
            return True
        except Exception as e:
            debug_logger(message=f"❌ Error writing to CSV file {file_path}: {e}", level="ERROR", **_get_log_args())
            return False
