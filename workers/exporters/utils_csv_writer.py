...
def write_scan_data_to_csv(
    ...
    if app_constants.global_settings["debug_enabled"]:
        debug_logger(
            message=f"Attempting to write scan data to CSV: {file_path}, append_mode={append_mode}. Let's save this data!",
            file=__file__,
    ...
    if output_dir and not os.path.exists(output_dir):
        try:
            ...
            if app_constants.global_settings["debug_enabled"]:
                debug_logger(
                    message=f"✅🔨 directory: {output_dir}. Path cleared!",
                    file=__file__,
                    version=current_version,
                    function=current_function,
                )
    ...
    try:
        ...
        with open(file_path, mode, newline="") as csv_file:
            ...
            if write_header:
                ...
                if app_constants.global_settings["debug_enabled"]:
                    debug_logger(
                        message=f"Wrote header to CSV file: {file_path}. Header added!",
                        file=__file__,
                        version=current_version,
                        function=current_function,
                    )
        ...
        if app_constants.global_settings["debug_enabled"]:
            debug_logger(
                message=f"Scan data written to CSV: {file_path}. Mission accomplished!",
                file=__file__,
                version=current_version,
                function=current_function,
            )
