...
def Marker_convert_SB_PDF_File_report_to_csv(pdf_file_path):
    ...
    if LOCAL_DEBUG: logger.debug(f"▶️ Starting PDF report conversion for '{os.path.basename(pdf_file_path)}'.",
        file=current_file,
    ...
    try:
        with pdfplumber.open(pdf_file_path) as pdf:
            ...
            if LOCAL_DEBUG: logger.debug(f"📄 Opened PDF with {len(pdf.pages)} pages.",
                file=current_file,
                version=current_version,
                function=current_function)
            ...
            for page_num, page in enumerate(pdf.pages):
                ...
                if LOCAL_DEBUG: logger.debug(f"🔍 Found {len(tables)} tables on Page {page_num + 1}.",
                    file=current_file,
                    version=current_version,
                    function=current_function)
                ...
                for table_num, table in enumerate(tables):
                    ...
                    for row_num, row in enumerate(table):
                        ...
                        try:
                            ...
                            logger.debug(f"↔️ PDF Freq conversion: '{frequency_pdf_str}' -> {freq_MHz_csv} MHz",
                                file=current_file,
                                version=current_version,
                                function=current_function)
                        except ValueError:
                        ...
                        logger.success(f"✅ Added PDF row: {csv_data[-1]}",
                            file=current_file,
                            version=current_version,
                            function=current_function)

        if LOCAL_DEBUG: logger.success(f"✅ Finished PDF report conversion. Extracted {len(csv_data)} rows.",
            file=current_file,
            version=current_version,
            function=current_function)
