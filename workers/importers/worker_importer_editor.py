...
def on_tree_double_click(importer_tab_instance, event):
    logger.debug(f"▶️ Treeview double-clicked for editing.",
    ...
    if col_index < 0 or col_index >= len(importer_tab_instance.tree_headers):
        logger.debug(f"⚠️ Invalid column index {col_index} for editing.",
            file=os.path.basename(__file__),
            version=importer_tab_instance.current_version,
        )
        return
...
def start_editing_cell(importer_tab_instance, item, col_index, initial_value=""):
    ...
    def on_edit_complete_and_navigate(event, navigate_direction=None):
        ...
        if row_idx < len(importer_tab_instance.tree_data) and isinstance(
            ...
            logger.success(f"✅ Updated cell: Row {row_idx+1}, Column '{importer_tab_instance.tree_headers[col_index]}' to '{new_value}'"
            )
            save_markers_file_internally(importer_tab_instance)
        else:
            logger.error(f"❌ Error: Row index {row_idx} out of bounds or data not a dictionary for self.tree_data.",
                file=os.path.basename(__file__),
                version=importer_tab_instance.current_version,
            )
...
def navigate_cells(importer_tab_instance, current_item, current_col_index, direction):
    logger.debug(f"▶️ Navigating cells.",
        file=os.path.basename(__file__),
        version=importer_tab_instance.current_version,
    )
    ...
    if current_row_idx == -1:
        logger.debug(f"🟡 Current item not found in tree for navigation.",
            file=os.path.basename(__file__),
            version=importer_tab_instance.current_version,
        )
        return
    if direction == "down":
        ...
        if next_row_idx < num_rows:
            ...
            if next_row_idx < len(importer_tab_instance.tree_data) and isinstance(
                ...
                logger.debug(f"🟡 Cannot Ctrl+Enter: No row below.",
                    file=os.path.basename(__file__),
                    version=importer_tab_instance.current_version,
                )
                return
    if next_item is not None and next_col_index != -1:
        if direction != "ctrl_down":
            try:
                ...
                if 0 <= next_col_index < len(next_item_values):
                    ...
                    logger.debug(f"🟡 Next column index {next_col_index} out of bounds for next item values. Setting empty.",
                        file=os.path.basename(__file__),
                        version=importer_tab_instance.current_version,
                    )
                    initial_value_for_next_cell = ""
                ...
                if LOCAL_DEBUG:
                    logger.exception("❌ Error getting initial value for next cell: . Setting empty.",
                        file=os.path.basename(__file__),
                        version=importer_tab_instance.current_version,
                    )
                initial_value_for_next_cell = ""
        ...
        logger.debug(f"🟡 No cell to navigate to in direction: {direction}",
            file=os.path.basename(__file__),
            version=importer_tab_instance.current_version,
        )
...
def on_tree_header_click(importer_tab_instance, event):
    logger.debug(f"▶️ Treeview header clicked for sorting.",
        file=os.path.basename(__file__),
        version=importer_tab_instance.current_version,
    )
    ...
    if region == "heading":
        ...
        col_index = int(column_id[1:]) - 1
        if col_index < 0 or col_index >= len(importer_tab_instance.tree_headers):
            logger.debug(f"⚠️ Invalid column index {col_index} for sorting.",
                file=os.path.basename(__file__),
                version=importer_tab_instance.current_version,
            )
            return
...
def sort_treeview(importer_tab_instance, column_name, ascending):
    logger.debug(f"▶️ Sorting treeview by '{column_name}', ascending: {ascending}.",
        file=os.path.basename(__file__),
        version=importer_tab_instance.current_version,
    )
    ...
    logger.success(f"✅ Sorted by '{column_name}' {'Ascending' if ascending else 'Descending'}."
    )
...
def delete_selected_row(importer_tab_instance, event):
    ...
    for item in selected_items:
        ...
        if index_in_tree < len(importer_tab_instance.tree_data):
            ...
            if LOCAL_DEBUG: logger.success(f"✅ Deleted row {index_in_tree + 1}.")
        else:
            logger.error(f"❌ Error: Row {index_in_tree + 1} not found in data."
            )
    save_markers_file_internally(importer_tab_instance)
