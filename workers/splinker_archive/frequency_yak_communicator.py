...
# 
#             if LOCAL_DEBUG:
#                 logger.success(f"🐐✅ YAK command dispatched. Sent {value_hz} Hz to {input_topic}.")
# 
#             self.update_all_from_device()
# 
#         except Exception as e:
#             logger.exception("❌ Error dispatching YAK command")
#             if LOCAL_DEBUG:
#                 logger.exception("❌ YAK dispatch failed! The error be")
# 
#     def update_all_from_device(self):
#         current_function_name = inspect.currentframe().f_code.co_name
#         if LOCAL_DEBUG:
#             logger.debug(f"🐐🟢 Triggering NAB_Frequency_settings to synchronize all 4 frequency values.")
# 
#         self.mqtt_controller.publish_message(
...
#         )
# 
#         if LOCAL_DEBUG: logger.success("✅ UPDATE ALL command sent to refresh frequency values from device.")
# 
...
#             if not gui_suffix:
#                 if LOCAL_DEBUG:
#                     logger.debug(f"🟡 Unknown YAK output suffix: {yak_suffix}. Ignoring.")
#                 return
# 
#             try:
...
# 
#                 if LOCAL_DEBUG:
#                     logger.success(f"🐐✅ YAK output processed. Synced {gui_suffix} with {value_mhz} MHz.")
#             except ValueError:
#                 if LOCAL_DEBUG:
#                     logger.debug(f"⚠️ Could not convert YAK output '{cleaned_value}' to float for topic {topic}. Skipping update.")
# 
#         except Exception as e:
#             logger.exception("❌ Error processing YAK output for {topic}")
#             if LOCAL_DEBUG:
#                 logger.exception("❌ NAB synchronization failed! The error be")
# 
